...
all_u  = cell(num_cases,1);
all_rho  = cell(num_cases,1);
%% 循环读取并截取前 5 秒
T_cut = 20;
for k = 1:num_cases
      fname = sprintf('6dot_%d.mat', k);
    S = load(fname, ...
        'tspan','e_q','e_dq','tau_mat','q_use','qd_mat','dq_use','dqd_mat','rho_mat','u_mat');
    ...
    % 性能函数上下界
    all_u{k}  =S.u_mat(idx(1:end-1), :);     % (M×6)
    all_rho{k}  = S.rho_mat(idx(1:end-1), :);          % (M×1)
end
%  t = all_tspan{1}(:);
...
h1=plot(t,  rho1,  '--', 'Color', colors(1,:), 'LineWidth', 1);
h2=plot(t, -rho1,  '--', 'Color', colors(1,:), 'LineWidth', 1);
   for k = 1:num_cases
...
for i = 1:6
    ax = subplot(3,2,i); hold(ax,'on');
     for k = 1:num_cases
            h1=plot(all_tspan{k}, all_u{k}(:,i), 'LineWidth',1.5, 'LineStyle', line_styles{1},'Color',colors(2,:));
            hold on;
