...
% persistent theta prev_t
% if isempty(theta), theta = zeros(2,1); prev_t    = t;  end
% Ts = max(eps, t - prev_t);
% theta = theta + 3*sign(e1)*Ts;          % d_u_est 可用简单滤波上界
s1     = e2 + 8*e1;
...
U_max2 = [10; 10]; 
% delta  = 0.05 * U_max2;            % 每个关节独立设定软区宽度（建议为其 5%）
% u_s = U_max2 .* tanh(u ./ delta);
...
function tau = softsat_tanh(u, tau_max, delta)
%SOFTSAT_TANH  向量化的 soft saturation (tanh 形式)
%
%   输入：
%     u        - 理想输入（n×1）
%     tau_max  - 饱和上限（n×1）
