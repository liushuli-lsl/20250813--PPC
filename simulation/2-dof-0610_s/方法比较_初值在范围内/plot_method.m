...
h_err = gobjects(num_cases,1);
    ...
    t = all_tspan{1}(:);
    rho1 = all_rho1{5}(:,1);  % 提取列向量
    x_fill = [t; flipud(t)];
    y_fill = [rho1; flipud(-rho1)];
fill(x_fill, y_fill, [0.5 0.5 0.5], ...  % 灰色grid off;
    'FaceAlpha', 0.1, ...
    'EdgeColor', 'none', ...
...
%     xline(2.5, 'LineStyle', line_styles{1},'Color',colors(7,:), 'LineWidth', 1.2, ...
%         'Label', 'T_p=2.5s', 'LabelOrientation', 'horizontal', ...
%         'Interpreter', 'latex');
...
%     xline(2.5, 'LineStyle', line_styles{1},'Color',colors(7,:), 'LineWidth', 1.2, ...
%         'Label', '$T_p=2.5$s', 'LabelOrientation', 'horizontal', ...
%         'Interpreter', 'latex');
for k = 1:num_cases
