...
p=0.3;
a=0.01;
% [rho1, drho1] = performance_poly(t, T_p, 0.8, a,p);
...
Ud_prev = zeros(n,1); d_prev = zeros(n,1);dz1=zeros(n,1);
cfg.feed_only = true;
gppf(t, n, dz1, Ud_prev, d_prev, cfg);  % 只更新LPF，不取rho
cfg = rmfield(cfg, 'feed_only');       % 避免污染后续

% 计算 G-PPF 及其导数（用于 BLF 精确补偿）
[rho1, drho1] = gppf(t,n, dz1, Ud_prev, d_prev,cfg);
