...
\begin{equation}
	...
	\label{eq:2}
\end{equation}
where $f(x)=-M^{-1}(q) (C(q,\dot{q})x_2 + G(q)) $, $g(q)=M^{-1}(q)$, $\omega(x,t)=-M^{-1}(q)(\Delta(q,\dot{q},t)-d(t))$, and $x_1=[x_{1,1},x_{1,2},...,x_{1,n}]^{\top}$, $x_2=[x_{2,1},x_{2,2},...,x_{2,n}]^{\top}$, $u$ is the saturation of control input $u=\tau$. Define the saturation error as
...
\begin{equation}\label{eq:gppf-def}
	\begin{cases}
\sigma_i(t) &= \operatorname{proj}\left( \sigma_0 + k_u r_{u,i} + k_d r_{d,i}, \left[ \sigma{\min}, \sigma{\max} \right] \right), \\
...
a\Bigl[g'(t)\,\Sigma_i(t)+g(t)\,\dot\Sigma_i(t)\Bigr], &t\ge T_p.
\end{cases}
\end{equation}
\begin{equation}
...
(\emph{i}) $\displaystyle\lim_{t\to0^+}\rho_i(t)=+\infty$, so any finite $e_i(0)$ satisfies $|e_i(0)|<\rho_i(0^+)$. (\emph{ii}) 由 $\kappa(1)=0$、$s(1)=0$ 得 $\rho_i(T_p^-)=a$；由 $s'(1)=0$ 与 $g(T_p)=g'(T_p)=0$ 得 $\dot\rho_i(T_p^-)=\dot\rho_i(T_p^+)=0$，实现 $C^1$ 级平滑衔接.
(\emph{iii}) For $t\ge T_p$, $\rho_i(t)=a[1+g(t)\Sigma_i(t)]\ge a$, with $g$ and $\Sigma_i$ bounded and low-pass filtered, thus $\rho_i$ remains positive and slowly varying.
