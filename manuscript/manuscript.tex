...
\begin{equation}
	M(q)\ddot{q} + C(q, \dot{q})\dot{q} + G(q)+ \Delta(q, \dot{q}, t)= u + d(t),
	\label{eq:1}
\end{equation}
...
For controller design, define the state $x = [x_1,x_2]^{\top} = [q,\dot{q}]^{\top} \in \mathbb{R}^{2n}$. The system \cref{eq:1} can be written in the standard state space form:
\begin{equation}
	\left\{
	\begin{aligned}
		\dot{x}_1 & = x_2,     \\
		\dot{x}_2 & =f(x) +g(q) u +\omega(x,t),
	\end{aligned}
	\right.
	\label{eq:2}
\end{equation}
where $f(x)=-M^{-1}(q) (C(q,\dot{q})x_2 + G(q)) $, $g(q)=M^{-1}(q)$, $\omega(x,t)=-M^{-1}(q)(\Delta(q,\dot{q},t)-d(t))$, and $x_1=[x_{1,1},x_{1,2},...,x_{1,n}]^{\top}$, $x_2=[x_{2,1},x_{2,2},...,x_{2,n}]^{\top}$, $u$ is the saturation of control input $u^{\prime}$. Define the saturation error as
\begin{equation}\label{eq:3}
\Delta u_{i} =  u^{\prime}_i-u_i=\begin{cases} 
	u^{\prime}_i-u_{\max}, & u^{\prime}_i \geq u_{\max}, \\
0,       & u_{\min} < u^{\prime}_i < u_{\max},  \quad i=1,2,...,n,\\
u^{\prime}_i-u_{\min}, & u^{\prime}_i \leq u_{\min},
\end{cases}
\end{equation}
...
\begin{equation}	\label{eq:7}
	\dot V(x) \le
	-\,\frac{\pi}{\eta\,T_p}\left(V(x)^{\,1-{\eta}/{2}}+V(x)^{\,1+{\eta}/{2}}\right)
	+\varsigma,
\end{equation}
...
\begin{equation}\label{eq:8}
	...
	\le
	\min\!\Biggl\{\,
	\left(\tfrac{2\,\eta\,T_p\,\varsigma}{\pi}\right)^{\frac{1}{1-{\eta}/{2}}}\!,
	\left(\tfrac{2\,\eta\,T_p\,\varsigma}{\pi}\right)^{\frac{1}{1+{\eta}/{2}}}
	\Biggr\} \Biggr\},
\end{equation}
...
\begin{definition}\label{def:PTS-ISS}
	...
	\dot V(x,u)\ \le\ -\,\frac{\pi}{\eta\,T}\Big(V^{\,1-{\eta}/{2}}+V^{\,1+{\eta}/{2}}\Big)\ +\ \sigma\big(\|u\|_\infty\big).
	\end{equation}
	则对任意初值与本质有界输入 $u$，在 $t=T$ 时
	\begin{equation}\label{eq:PTS-ISS-bound}
	V(T)\ \le\ \min\!\left\{
	\Big(\tfrac{2\eta T}{\pi}\,\sigma(\|u\|_\infty)\Big)^{\!\frac{1}{\,1-{\eta}/{2}\,}},
	\ \ 
	\Big(\tfrac{2\eta T}{\pi}\,\sigma(\|u\|_\infty)\Big)^{\!\frac{1}{\,1+{\eta}/{2}\,}}
	\right\},
	\end{equation}
	...
	\[
	\dot V\ \le\ -\,\frac{\pi}{\eta\,T}\Big(V^{\,1-{\eta}/{2}}+V^{\,1+{\eta}/{2}}\Big)\ +\ \sigma(r).
	\]
	考虑比较系统
	\[
	\dot y\ =\ -\,\frac{\pi}{\eta\,T}\Big(y^{\,1-{\eta}/{2}}+y^{\,1+{\eta}/{2}}\Big)\ +\ \sigma(r),\quad y(0)=V(0).
	\]
	由比较引理得 $V(t)\le y(t)$。对该标量系统应用\cref{lemma:1}，将常数项 $\varsigma$ 替换为 $\sigma(r)$, 即可得到 \cref{eq:PTS-ISS-bound}。进一步令
\[
V^\star(r)=\min\!\left\{
\Big(\tfrac{2\eta T}{\pi}\,\sigma(r)\Big)^{\!\frac{1}{1-{\eta}/{2}}},
\ \ 
\Big(\tfrac{2\eta T}{\pi}\,\sigma(r)\Big)^{\!\frac{1}{1+{\eta}/{2}}}
\right\},
\]
...
若 $u=0$, 则 $V^\star(0)=0$ 且 $V(T)=0$。证毕。
	...
	\begin{remark}
	与原\cref{Lemma:1}对比, \cite{WangEtAl_2022_Adaptivefuzzy} 要求 $\dot V \le -\frac{\pi}{\eta T_p}(V^{1-\eta/2}+V^{1+\eta/2})+\varsigma$，其中 $\varsigma$ 为常数.
...
\kappa(b) =\nu_1(-\ln b)^{\nu_2},   
\end{equation}
which ensures $\kappa(b)\to+\infty$ as $b\to0^+$ and $\kappa(1)=0$, $\nu_1>0, \nu_2 \geq 1$.
 
For each channel $i=1,\dots,n$, we define the global performance function
\begin{equation}\label{eq:gppf-def}
\rho_i(t)=
\begin{cases}
\displaystyle a + \sigma_i(t)\,\kappa\left(b(t)\right)\,s\left(b(t)\right), & 0<t<T_p,\\[6pt]
\displaystyle a\Bigl[1 + c(t)\,\Sigma_i(t)\Bigr], & t\ge T_p,
\end{cases}
\end{equation}
...
\begin{equation}\label{eq:gppf-def}
	\begin{cases}
\sigma_i(t) = \operatorname{proj}\left( \sigma_0 + k_u r_{u,i} + k_d r_{d,i}, \left[ \sigma_{\min}, \sigma_{\max} \right] \right), \\
\Sigma_i(t) = \operatorname{proj}\left( k_u r_{u,i} + k_d r_{d,i} + k_e r_{e,i}, \left[ 0, \Sigma_{\max} \right] \right),
\end{cases}
\end{equation}
其中，投影算子定义为：$\operatorname*{proj}(x,[\ell, u]) = \min\{\max\{x, \ell\}, u\}$，其目的是为了对自适应参数进行限幅，$0<\sigma_0<\sigma_{\min}<\sigma_{\max}$，$\Sigma_{\max}>0$，$\sigma_{0}>0$，$k_u>0$，$k_d>0$，$\tau_u>0$，$\tau_d>0$。对三类残差进行一阶滤波
C
\begin{equation}\label{eq:gppf-def}
	\begin{cases}
\tau_u \dot{r}_{u,i} = -r_{u,i} + \left| \Delta u_i \right|, \\
\tau_d \dot{r}_{d,i} = -r_{d,i} + \left| d_i \right|, \\
\tau_e \dot{r}_{e,i} = -r_{e,i} + \left| e_i \right|, \\
\end{cases}
\end{equation}
...
c(t)=1-\exp\!\left(-\iota (t-T_p)^2\right),
\end{equation}      
where $\iota>0, c(T_p)=0, c'(T_p)=0$. 

Since $\dot b=\frac{p}{T_p}(\frac{\max{t,t_\epsilon }}{T_p})^{p-1}$, $\kappa'(b)=-1/b$, $s'(b)=-6b+6b^2$,  and \cref{eq:gppf-def}. there is
\begin{equation}\label{eq:gppf-def}
	\dot{\rho}_i(t)=
	\begin{cases}
   \dot\sigma_i\,\kappa(b)\,s(b) + \sigma_i \Bigl[\kappa'(b)\,s(b)+\kappa(b)\,s'(b)\Bigr]\dot b, &0<t<T_p,\\
a\Bigl[c'(t)\,\Sigma_i(t)+c(t)\,\dot\Sigma_i(t)\Bigr], &t\ge T_p.
\end{cases}
\end{equation}
\begin{equation}
\dot{\Sigma}_{i} = k_{u}\dot{r}_{u,i} + k_{d}\dot{r}_{d,i} + k_{e}\dot{r}_{e,i} 
= \frac{k_{u}}{\tau_{u}}(-r_{u,i} + |\Delta u_{i}|) 
+\frac{k_{d}}{\tau_{d}}(-r_{d,i} + |d_{i}|) 
+\frac{k_{e}}{\tau_{e}}(-r_{e,i} + |e_{i}|). 
\end{equation}
...
(\emph{i}) $\displaystyle\lim_{t\to0^+}\rho_i(t)=+\infty$, so any finite 系统初始误差$e_i(0)$ satisfies $|e_i(0)|<\rho_i(0^+)$. (\emph{ii}) 由 $\kappa(1)=0$、$s(1)=0$ 得 $\rho_i(T_p^-)=a$；由 $s'(1)=0$ 与 $c(T_p)=c'(T_p)=0$ 得 $\dot\rho_i(T_p^-)=\dot\rho_i(T_p^+)=0$，实现 $C^1$ 级平滑衔接.
(\emph{iii}) For $t\ge T_p$, $\rho_i(t)=a[1+c(t)\Sigma_i(t)]\ge a$, with $c$ and $\Sigma_i$ bounded and low-pass filtered, thus $\rho_i$ remains positive and slowly varying.
(\emph{iv}) 通过约束$\sigma\in[\sigma{\min},\sigma{\max}]$和$\Sigma\in[0,\Sigma{\max}]$实现，$\rho, \dot{\rho}$有界且变化缓慢，对闭环注入高频激励。
...
% The structure of the proposed control algorithm is shown in \cref{fig:2}.
% \begin{figure}[H]
% 	\centering
...
为将饱和残差在预设时间 $T_\zeta<cT_p$（$0<c<1$）内抽干， 引入动态补偿信号 $\zeta$:
\begin{equation}\label{eq:zeta2-pts}
	\begin{cases}
\dot\zeta \ =\ \delta\ -\ \mu\,\zeta\ -\ M^{-1}(q)\Delta u,\\
\delta\ =\ -\frac{\pi}{2\gamma T_\zeta}\left(|\zeta|^{1- \gamma}\mathrm{sgn}(\zeta)+|\zeta|^{1+\gamma}\mathrm{sgn}(\zeta)\right),
	\end{cases}
\end{equation}
...
记
$
P_{i}=\frac{\rho_{1,i}^4}{(\rho_{1,i}^2-z_{1,i}^2)^2}>0,
Q_{i}=\frac{ z_{1,i}^4}{(\rho_{1,i}^2-z_{1,i}^2)^2}>0,
 \rho_{1,i}^2 - z_{1,i}^2 \le \rho_{1,i}^2,
\varGamma_{i}=\frac{z_{1,i}\left(\rho_{1,i}^{2}-z_{1,i}^{2}\right)}{\rho_{1,i}^{4}}
$. 将$\dot z_{1,i}= z_{2,i}-\dot q_{d,i}+\alpha^{f}_i+\zeta_i$,代入 BLF 导数后，并根据公式（），得到
$$
\dot V_1=\sum_{i=1}^n\!\Big[P_{i} z_{1,i}z_{2,i}+P_{i}z_{1,i}(\alpha_i+\zeta_i-\dot q_{d,i})-P_{i}z_{1,i}\tilde\alpha_i-Q_{i}\rho_{1,i} \dot\rho_{1,i}\Big]
$$
...
-\mathcal{K}_{1,i}(z_{1,i},\rho_{1,i})\varGamma_{i}
-k_{1,i} \varGamma_{i},
\end{equation}
where $k_1=\mathrm{diag}(k_{1,1},k_{1,i},\cdots,k_{1,n})$, $k_{1,i}>0$, $
\mathcal{K}_{1,i}(z_{1,i},\rho_{1,i})
=\frac{\pi \rho_{1,i}^2}{2\eta T_p}\left(V_{1,i}^{-\eta/2}+n^{-{\eta}/{2} }V_{1,i}^{\eta/2}\right)$.

则一步 Lyapunov 导数化简为

$$
\dot V_1
\le \sum_{i=1}^n \left( P_{i}z_{1,i} z_{2,i}-
\frac{2}{\rho_{1,i}^2} V_{1,i} \mathcal{K}_{1,i}- \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i}-P_{i}z_{1,i}\tilde\alpha_i
\right).
$$
...
\begin{equation}\label{eq:25}
	V_2= V_1+ \frac{1}{2}\sum_{i=1}^{n}z_{2,i}^{\top}z_{2,i}. 
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\dot V_2& = \dot V_1+ \sum_{i=1}^{n}z_{2,i} \dot{z}_{2,i}
		=  \dot V_1+\sum_{i=1}^n \Big[z_{2,i}\,(\dot x_{2,i}-\dot\alpha_i^{f}-\dot\zeta_i)\Big]\\
		&
		 \le  \sum_{i=1}^n \left[P_{i}z_{1,i} z_{2,i}+ z_{2,i}\,\left(f_i(x)+\omega_i(x,t)+g_i(u^{\prime}_i-\Delta u_i)-\dot\alpha_i^{f}-\delta_i\ +\ \mu\,\zeta_i\ +\ g_i\,\Delta u_i \right)\right]
	\end{aligned}
\end{equation}
...
\begin{aligned}
	u^{\prime} =&C(q,\dot{q})x_2 + G(q)+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-P_1 z_1-\mathcal{K}_{2}(z_{2})
-\hat \omega
-k_s\,\mathrm{sgn}(z_{2})-k_{2}z_2\right]
\end{aligned}
\end{equation}
where $k_2=\mathrm{diag}(k_{2,1},k_{2,i},\cdots,k_{2,n})$, $k_{2,i}>0$, $k_s>0$, $\mathcal{K}_{2}(z_{2})=\frac{\pi}{2\eta T_p}\left[ 2^{\eta/2} | z_2\vert ^{1-\eta}\mathrm{sgn}(z_2)+(\frac{n}{2})^{-{\eta}/{2} }| z_2\vert ^{1+\eta}\mathrm{sgn}(z_2)\right]$.
...
% 		&
% 		 = \sum_{i=1}^n \left[\,P_{2,i}z_{2,i}\,\left(f_i(x)+\omega_i(x,t)+g_i(u^{\prime}_i-\Delta u_i)-\dot\alpha_i^{f}-\delta_i\ +\ \mu\,\zeta_i\ +\ g_i\,\Delta u_i-\tfrac{z_{2}^{3}}{\rho_{2,i}^{3}}\dot\rho_{2,i} \right)\right]
% 	\end{aligned}
% \end{equation}

% Let $Q_2=\mathrm{col}\{Q_{2,i}\}$,
% $P_1=\mathrm{col}\{P_{i}\}$,
% $\Phi_2=\mathrm{col}\{\varGamma_{2,i}\}$. The control input torque is designed as
% \begin{equation}\label{eq:tau-cmd}
% \begin{aligned}
% 	u^{\prime} =&C(q,\dot{q})x_2 + G(q)\\
% &+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-\frac{P_1}{P_2}z_1+\tfrac{z_{2}^{3}}{\rho_{2}^{3}}\dot\rho_{2}-\mathcal{K}_{2}(z_{2},\rho_{2}) \varGamma_{2}
% -\hat \omega
% -k_s\,\mathrm{sgn}(z_{2})-k_{2}\rho_{2}^2 \varGamma_{2}\right]
% \end{aligned}
% \end{equation}
...
% u =&C(q,\dot{q})x_2 + G(q)\\
% &+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-\frac{P_1}{P_2}z_1+\tfrac{z_{2}^{3}}{\rho_{2,i}^{3}}\dot\rho_{2,i}-\mathcal{K}_{2}(z_{2},\rho_{2,i}) \varGamma_{2}
% -\hat{h}(\chi)
% -\hat \omega
% -k_s\,\mathrm{sgn}(z_{2})-k_{2}\rho_{2,i}^2 \varGamma_{2,i}\right]
...
% +\Lambda\left(\hat{\dot x}_2 - f(x) - M^{-1}(q)\,u - \hat h(\chi)\right),
% \end{cases}
% \end{equation}
...
\hat{\dot x}_2=\omega_d(x_2-\vartheta),   \\
\dot{\hat \omega}= -\Lambda\,\hat \omega
+\Lambda\left(\hat{\dot x}_2 - f(x) -g(q)\,u \right),
\end{cases}
\end{equation}
...
% $\| \varepsilon_d \Vert  \le \bar{\epsilon}_d, \bar{\epsilon}_d>0$, $\| \omega\Vert  \le \bar{\omega}, \bar{\omega}>0$, $\| \dot{\omega}\Vert  \le \bar{\dot{\omega}}, \bar{\dot{\omega}}>0$.
...
\begin{equation}
	\begin{aligned}
	\dot V_2
\le&\sum_{i=1}^n \left(-\frac{2}{\rho_{1,i}^2} V_{1,i} \mathcal{K}_{1,i}- \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i}-P_{i}z_{1,i}\tilde\alpha_i
\right)+\sum_{i=1}^n \left( -z_{2,i}\mathcal{K}_{2,i}-z_{2,i}\tilde{\omega}_i - k_{2,i} z_{2,i}^2 \right) -k_s\sum_{i=1}^n z_{2,i}\mathrm{sgn}(z_2)
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		\mathcal{L}
	=   V_{2}+\tfrac{1}{2}\| \tilde\alpha\Vert ^2+\tfrac{1}{2}\| \tilde \omega\Vert ^2
	\end{aligned}
\end{equation} 
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		\dot{\mathcal{L}}
		\le &\sum_{i=1}^n \left(-\frac{2}{\rho_{1,i}^2} V_{1,i} \mathcal{K}_{1,i}- \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i}-P_{i}z_{1,i}\tilde\alpha_i
\right)+\sum_{i=1}^n \left( -z_{2,i}\mathcal{K}_{2,i}-z_{2,i}\tilde{\omega}_i - k_{2,i} z_{2,i}^2 \right) -k_s\sum_{i=1}^n P_{2,i} \left\lvert z_{2,i}\right\rvert +\tilde\alpha^\top \dot{\tilde \alpha}+\tilde \omega^\top \dot{\tilde \omega} \\
		\le &\sum_{i=1}^n \left( -\frac{2}{\rho_{1,i}^2} V_{1,i} \mathcal{K}_{1,i}-z_{2,i}\mathcal{K}_{2,i}\right)
		- \sum_{i=1}^n \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i} - \sum_{i=1}^n k_{2,i} z_{2,i}^2 +\tilde\alpha^\top \dot{\tilde \alpha}-\sum_{i=1}^n P_{i}z_{1,i}\tilde\alpha_i
		-\sum_{i=1}^n z_{2,i}\tilde{\omega}_i +\tilde \omega^\top \dot{\tilde \omega}-k_s\sum_{i=1}^n \left\lvert z_{2,i}\right\rvert
	\end{aligned}
\end{equation}
...
% \begin{aligned}
% -\Psi(V_{1,i})-P_{i}z_{1,i}\tilde\alpha_i
...
$
|P_{i}z_{1,i}| \le c_v |z_{1,i}|\le c_v \sqrt{2V_{1,i}}
$. Then, we get
$$
|P_{i}z_{1,i}\tilde\alpha_i|
...
\begin{aligned}
	\mathcal{L}_{\alpha}=&-\sum_{i=1}^n P_{i}z_{1,i}\tilde\alpha_i
- \sum_{i=1}^n \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i}
+\tilde\alpha^\top\dot{\tilde\alpha}\\
\le & \varepsilon \sum_{i=1}^n V_{1,i}+\frac{c_v^{\,2}}{2\varepsilon}\sum_{i=1}^n \|\alpha\Vert^2- \sum_{i=1}^n \frac{2}{\rho_{1,i}^2}k_{1,i}V_{1,i}-\frac{1}{2\beta}\|\tilde\alpha\|^2+\beta\,\mathcal{E}(t)\\
\le &
\sum_{i=1}^n \left(\varepsilon-\frac{2}{\rho_{1,i}^2}k_{1,i}\right) V_{1,i}
+\left(\frac{c_v^{\,2}}{2\varepsilon}-\frac{1}{2\beta}\right)\|\alpha\Vert^2
+\beta\,\mathcal{E}(t)\\
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:50}
	\begin{aligned}
\mathcal{L}_\omega
=&\,-\sum_{i=1}^n z_{2,i}\tilde{\omega}_i\;+\;\tilde \omega^\top \dot{\tilde \omega}\;-\;k_s\sum_{i=1}^n | z_{2,i} |  \\
=&\;-\tilde\omega^\top\Lambda\tilde\omega\;-\sum_{i=1}^n z_{2,i}\tilde\omega_i\;-\;k_s\sum_{i=1}^n  | z_{2,i} |\;+\;\tilde\omega^\top\Delta_\omega .
\end{aligned}
\end{equation}
...
$$
\big|\tilde\omega^\top z_2 \big|
=\big|(\Lambda^{1/2}\tilde\omega)^\top\Lambda^{-1/2} z_2\big|
\le \tfrac{\epsilon_1}{2}\tilde\omega^\top\Lambda\tilde\omega
+ \tfrac{1}{2\epsilon_1}\big\|\Lambda^{-1/2} z_2\big\|^2,
$$
...
+\sum_{i=1}^n\!\left[\tfrac{1}{2\epsilon_1\,\lambda_{i}}z_{2,i}^2
- k_s |z_{2,i}|\right]
+\tfrac{1}{2\epsilon_2}\big\|\Lambda^{-1/2}\Delta_\omega\big\|^2 .
$$
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		\dot{\mathcal{L}} 
\le&   \sum_{i=1}^n \left( -\frac{2}{\rho_{1,i}^2} V_{1,i} \mathcal{K}_{1,i}-z_{2,i}\mathcal{K}_{2,i}\right) +\sum_{i=1}^n \left(\varepsilon-\frac{2}{\rho_{1,i}^2}k_{1,i}\right) V_{1,i}
+\left(\frac{c_v^{\,2}}{2\varepsilon}-\frac{1}{2\beta}\right)\|\alpha\Vert^2
+\beta\,\mathcal{E}(t) - k_{2,i}\sum_{i=1}^n z_{2,i}^2\\
&-\Big(1-\tfrac{\epsilon_1+\epsilon_2}{2}\Big)\lambda_{\min}\|\tilde\omega\|^2\;+\;\tfrac{1}{2\epsilon_2}\big\|\Lambda^{-1/2}\Delta_\omega\big\|^2              
\\
\le&\sum_{i=1}^n \left(\varepsilon-\frac{2}{\rho_{1,i}^2}k_{1,i}\right) V_{1,i}- k_{2,i}\sum_{i=1}^n z_{2,i}^2-\Big(\frac{1}{2\beta}-\frac{c_v^{\,2}}{2\varepsilon}\Big)\|\tilde\alpha\|^2 -\Big(1-\tfrac{\epsilon_1+\epsilon_2}{2}\Big)\lambda_{\min}\|\tilde\omega\|^2
+\frac{1}{2\epsilon_2}\big\|\Lambda^{-1/2}\Delta_\omega\big\|^2
+\beta\,\mathcal{E}(t)
% 		\le&\sum_{i=1}^n \left( - k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}-2 V_{1,i} \mathcal{K}_{1,i}\right)
% 		+\sum_{i=1}^n \left( - k_{2,i}\frac{\rho_{2,i}^2 z_{2,i}^2}{\rho_{2,i}^2-z_{2,i}^2}-z_{2,i}\mathcal{K}_{2,i} \right)\\
...
% \le&\sum_{i=1}^n \left( - k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}-2 V_{1,i} \mathcal{K}_{1,i}\right)
% 		+\sum_{i=1}^n \left( - k_{2,i}\frac{\rho_{2,i}^2 z_{2,i}^2}{\rho_{2,i}^2-z_{2,i}^2}-z_{2,i}\mathcal{K}_{2,i} \right)\\
% 		&+\frac{\varepsilon}{2}\sum_{i=1}^n V_{1,i}+\frac{c_v^{\,2}}{2\varepsilon}\|\tilde\alpha\|^2-\frac{1}{2\beta}\|\tilde\alpha\|^2+\beta\,\mathcal{E}(t)-\Big(1-\tfrac{\epsilon_1+\epsilon_2}{2}\Big)\lambda_{\min}\|\tilde\omega\|^2\;+\;\tfrac{1}{2\epsilon_2}\big\|\Lambda^{-1/2}\Delta_\omega\big\|^2
	\end{aligned}
\end{equation}
...
% 	\begin{aligned}
% 		\dot{\mathcal{L}} 
% \le&-(2k_{1,i}-\varepsilon)\sum_{i=1}^n V_{1,i}- k_{2,i}\sum_{i=1}^n z_{2,i}^2-\Big(\frac{1}{2\beta}-\frac{c_v^{\,2}}{2\varepsilon}\Big)\|\tilde\alpha\|^2 -\Big(1-\tfrac{\epsilon_1+\epsilon_2}{2}\Big)\lambda_{\min}\|\tilde\omega\|^2
% +\frac{1}{2\epsilon_2}\big\|\Lambda^{-1/2}\Delta_\omega\big\|^2
% +\beta\,\mathcal{E}(t)
% 	\end{aligned}
% \end{equation}
...
\begin{aligned}
\mathcal{M}  = \min \left\{
	\min_{i}\left(\frac{2}{\rho_{1,i}^2}k_{1,i}-\varepsilon\right), \min_{i}\{2k_{2,i}\}, \min_{i}\left(\frac{1}{\beta}-\frac{c_v^{\,2}}{\varepsilon}\right),\min_{i}\left(\frac{1}{2}-\epsilon_1-\epsilon_2\right)
\right\}>0,
\end{aligned}
\end{equation}
...
From the above \cref{eq:53,eq:54,eq:55,eq:56}, as long as the controller parameters are chosen reasonably, $\mathcal{L}$ is 一致有界并可收敛, which implies all signals $z_{1}, z_{2}, {\tilde{\alpha}}, \tilde{\omega}$ are bounded within the compact set $\varPhi $, 从而存在常数$\phi>0$ such that
\begin{equation}\label{eq:57}
	(z_1,z_2,\tilde\alpha,\tilde{\omega}) \in \varPhi =\left\{
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		\dot{\mathcal{L}} 
\le& -\frac{\pi}{ \eta T_p }\sum_{i=1}^n \left[(V_{1,i})^{1-\eta/2}+n^{-{\eta}/{2} }(V_{1,i})^{1+{\eta/2}}\right]-\frac{\pi}{\eta T_p}\sum_{i=1}^n \left[ \left(\frac{1}{2} z_{2,i}^2\right)^{1-{\eta}/{2}}+n^{-{\eta}/{2}}\left(\frac{1}{2} z_{2,i}^2\right)^{1+{\eta}/{2}}\right]\\
&-\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left[ \left(\tfrac{1}{2}\tilde\alpha^2 \right)^{1-{\eta}/{2}}+\left(\tfrac{1}{2}\tilde \omega^2\right)^{1-{\eta}/{2}}\right]  - n^{{\eta}/{2}}\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left[ \left(\tfrac{1}{2}\tilde\alpha^2\right)^{1+{\eta}/{2}}+\left(\tfrac{1}{2}\tilde \omega^2\right)^{1+{\eta}/{2}}\right] \\
& +\frac{2\pi}{\eta T_{p}} \sum_{i=1}^{n}\left[  \left(\frac{\phi}{2}\right)^{1-{\eta}/{2}} + n^{{\eta}/{2}}\left(\frac{\phi}{2}\right)^{1+{\eta}/{2}}  \right]+\mathcal{N}(t)   \\
\le &-\frac{\pi}{\eta T_{p}}\left[ \sum_{i=1}^{n}V_{1,i}+\sum_{i=1}^{n}\frac{1}{2} z_{2,i}^2 +\left(\tfrac{1}{2}\tilde\alpha^2 \right)+\left(\tfrac{1}{2}\tilde \omega^2\right)\right]^{1-{\eta}/{2}}-\frac{\pi}{\eta T_{p}}\left[ \sum_{i=1}^{n}V_{1,i}+\sum_{i=1}^{n}\frac{1}{2} z_{2,i}^2 +\left(\tfrac{1}{2}\tilde\alpha^2 \right)+\left(\tfrac{1}{2}\tilde \omega^2\right)\right]^{1+{\eta}/{2}} \\
&+\frac{2\pi}{\eta T_{p}}\sum_{i=1}^{n} \left[  \left(\frac{\phi}{2}\right)^{1-{\eta}/{2}} + n^{{\eta}/{2}}\left(\frac{\phi}{2}\right)^{1+{\eta}/{2}}  \right]+\mathcal{N}(t) \\
&  \le-\frac{\pi}{\eta T_{p}}\left( \mathcal{L}^{1-{\eta}/{2}}+\mathcal{L}^{1+{\eta}/{2}}\right)+\mathcal{R}(t) 
	\end{aligned}
\end{equation}
where
\begin{equation}\label{eq:61}
	\begin{aligned}
\mathcal{R}(t) =\frac{2\pi}{\eta T_{p}}\sum_{i=1}^{n} \left[  \left(\frac{\phi}{2}\right)^{1-{\eta}/{2}} + n^{{\eta}/{2}}\left(\frac{\phi}{2}\right)^{1+{\eta}/{2}}  \right]+\mathcal{N}(t).
		\end{aligned}
\end{equation}
...
$$
\dot y= -\frac{\pi}{\eta T_p}\Big(y^{1-{\eta}/{2}}+y^{1+{\eta}/{2}}\Big)+\overline{\mathcal{R}}_t,\qquad y(0)=\mathcal{L}(0).
$$
...
=\min\!\left\{
\Big(\tfrac{2\eta T_p\,\overline{\mathcal{R}}_{T_p}}{\pi}\Big)^{\!\frac{1}{1-{\eta}/{2}}},
\ \Big(\tfrac{2\eta T_p\,\overline{\mathcal{R}}_{T_p}}{\pi}\Big)^{\!\frac{1}{1+{\eta}/{2}}}
\right\}.
$$
...
% 	\min\left\{
% 	\left(\tfrac{2\eta\,T_p \mathcal{R}(t) }{\pi}\right)^{\!{\frac{1}{1-{\eta}/{2}}}}\!,
% 	\left(\tfrac{2\eta\,T_p \mathcal{R}(t) }{\pi}\right)^{\!{\frac{1}{1+{\eta}/{2}}}}
% 	\right\}\right\}.
% \end{equation}
...
\begin{theorem}\label{thm:u-only}
	...
	对$\mathcal{L}_{\zeta,i}$求时间导数。并结合$\zeta_i\,\mathrm{sgn}(\zeta_i)^\alpha=|\zeta_i|^{\alpha+1}$ 得
	...
	$$
	|\zeta_i(g \Delta u)_i|\ \le\ \frac{\mu}{2}\zeta_i^2+\frac{1}{2\mu}(g \Delta u)_i^2 .
	$$
	...
	\ \le\
	-\frac{\pi}{2\gamma T_\zeta}\left[(\mathcal{L}_{\zeta,i})^{1-{\gamma}/{2}}+(\mathcal{L}_{\zeta,i})^{1+{\gamma}/{2}}\right]
	\ +\ \sum_{i=1}^{n}\frac{1}{2\mu}(g \Delta u)_i^2 .
	$$
	...
	\dot{\mathcal{L}}_{\zeta,i}
	\ \le\
	-\frac{\pi}{\gamma T_\zeta}\Big(\mathcal{L}_{\zeta,i}^{1-{\gamma}/{2}}+\mathcal{L}_{\zeta,i}^{1+{\gamma}/{2}}\Big)
	...
	\min\!\left\{
	\Big(\tfrac{2\gamma T_\zeta\,\varphi (\|\Delta u\|_\infty)}{\pi}\Big)^{\!\frac{1}{1-{\gamma}/{2}}},
	\ \ 
	\Big(\tfrac{2\gamma T_\zeta\,\varphi (\|\Delta u\|_\infty)}{\pi}\Big)^{\!\frac{1}{1+{\gamma}/{2}}}
	\right\},
	$$
...
%   \ge
%   \big(\zeta_{1,i}^{2}+\zeta_{2,i}^{2}\big)^{1\pm{\gamma}/{2}}
%   = (2W_i)^{1\pm{\gamma}/{2}},
% \)
...
%   \Big[(2W_i)^{1-{\gamma}/{2}}+(2W_i)^{1+{\gamma}/{2}}\Big]
%   \le
%   -\frac{\pi}{2\gamma T_{\zeta}}
%   \Big[W_i^{1-{\gamma}/{2}}+W_i^{1+{\gamma}/{2}}\Big].
% \end{equation}
...
%   &\le
%   -\frac{\pi}{2\gamma T_{\zeta}}
%    \sum_{i=1}^{n}\Big( W_i^{1-{\gamma}/{2}}+W_i^{1+{\gamma}/{2}} \Big)
...
%   \min\!\left\{
%     \Bigl(\frac{2\gamma T_{\zeta}\Lambda}{\pi}\Bigr)^{\!\frac{1}{\,1-{\gamma}/{2}\,}},
%     
%     \Bigl(\frac{2\gamma T_{\zeta}\Lambda}{\pi}\Bigr)^{\!\frac{1}{\,1+{\gamma}/{2}\,}}
%   \right\}.
% \end{equation}
...
% %   \sqrt{2}\,
% %   \min\!\left\{
% %     \Bigl(\frac{2\gamma T_{\zeta}\Lambda}{\pi}\Bigr)^{\!\frac{1}{\,2\left(1-{\gamma}/{2}\right)}},
% %     
% %     \Bigl(\frac{2\gamma T_{\zeta}\Lambda}{\pi}\Bigr)^{\!\frac{1}{\,2\left(1+{\gamma}/{2}\right)}}
% %   \right\}.
% % \]
...
% % 	\ge
% % 	\left(\zeta_{1}^{2}+\zeta_{2}^{2}\right)^{1\pm{\gamma}/{2}}
% % 	=(2V)^{1\pm{\gamma}/{2}}$, 因此
	
% % 	\begin{equation}\label{eq:65}
% % 	\zeta_{1}\delta_{1}+\zeta_{2}\delta_{2}
% % 	\le
% % 	-\frac{\pi}{4\gamma T_{\zeta}}\Bigl[(2V)^{1-{\gamma}/{2}}+(2V)^{1+{\gamma}/{2}}\Bigr]
% % 	\le-\frac{\pi\,2^{1-\gamma/2}}{4 \gamma T_{\zeta}}
% % 	\Bigl(V^{1-{\gamma}/{2}}+V^{1+{\gamma}/{2}}\Bigr)\le -\frac{\pi}{2\gamma T_{\zeta}}
% % 	\Bigl(V^{1-{\gamma}/{2}}+V^{1+{\gamma}/{2}}\Bigr).
% % \end{equation}
...
% % 	\Bigl(V^{1-{\gamma}/{2}}+V^{1+{\gamma}/{2}}\Bigr)- \left(\frac{1-\mu_{1}}{2}\right)\zeta_{1}^{2}
...
% % 	&\le
% % 	-\frac{\pi}{2\gamma T_{\zeta}}
% % 	\Bigl(V^{1-{\gamma}/{2}}+V^{1+{\gamma}/{2}}\Bigr)+\varLambda ,
% % \end{aligned}
% % \end{equation}
...
% % 	\begin{equation}\label{eq:70}
% % 	\varLambda 
% % 	=\frac{2\pi}{\eta T_{p}} \left[  \left(\frac{\aleph}{2}\right)^{1-{\eta}/{2}} + \left(\frac{\aleph}{2}\right)^{1+{\eta}/{2}}  \right],
% % \end{equation}
...
\( r_5 = m_2 l_{c2} \).
Here, \(m_i\), \(I_i\), and \(l_i\) are the mass, moment of inertia, and length of the \(i\)th link, respectively, and \(l_{ci}\) is the distance from the $(i-1)$ joint to the center of mass of the \(i\)th link; The mass and inertia parameters are set as: \( m_1 = 1 \ \mathrm{kg} \), \( m_2 = 0.75 \ \mathrm{kg} \), \( l_1 = 0.35 \ \mathrm{m} \), \( l_2 = 0.31 \ \mathrm{m} \), \( l_{c1} = 0.175 \ \mathrm{m} \), \( l_{c2} = 0.155 \ \mathrm{m} \), \( I_1 = 0.25 m_1 l_1^2 \ \mathrm{kg \, m^2} \), \( I_2 = 0.25 m_2 l_2^2 \ \mathrm{kg \, m^2} \), \( g = 9.8 \ \mathrm{m/s^2} \).
The control objective is to design the control torque \( u \) so that the joints track a desired joint-space trajectory within prescribed performance and a predefined-time. The initial velocity is $\dot{x}(0)=[\dot{q_1}(0),\dot{q_2}(0)]^{\top}=[0,0]^{\top}$ rad/s. The desired joint trajectories are \( q_{d} = [ 0.1 \sin(4.5\,t)+\cos(4.5\,t), \, 0.1 \sin(4.5\,t)+\cos(4.5\,t)]^{\top} \), \( \dot{q}_d = [ 0.45\,\cos\left(4.5\,t\right) - 4.5\,\sin\left(4.5\,t\right), 0.45\,\cos\left(4.5 t\right) - 4.5\,\sin\left(4.5\,t\right)]^{\top} \).
% \begin{equation}
% 	\begin{aligned}
% 		 & M_0(q) =
% 		\begin{bmatrix}
...
The prescribed performance horizon and shaping are \(T_p=3\,\mathrm{s}\), \(p=0.5\), $t_\epsilon=0.01$, $\nu_1=2, \nu_2=1$, with steady tube floor \(a=0.01\). 
The G-PPF adapts online using low-pass filtered residuals \(|\Delta u|,|d|,|e|\) with \((\tau_u,\tau_d,\tau_e)=(0.05,0.1,0.08)\) and gains \((k_u,k_d,k_e)=(1.8,0.3,0.8)\); the pre-phase gain is limited by \(\sigma_0=1.0\), \([\sigma_{\min},\sigma_{\max}]=[1.2,2]\); the post-phase amplitude is capped by \(\Sigma_{\max}=20\) and \(\iota=3\).
The controller parameters are set as: \(\eta=0.1\), \(\beta=0.2\), \(k_1=\mathrm{diag}(36,30)\), \(k_2=\mathrm{diag}(12,12)\).
The dynamic anti-saturation compensator \(\zeta\) adopts \(T_\zeta=1.8\,\mathrm{s}\), \(\gamma=0.4\), \(\mu=14\).
The observer uses \(\omega_d=0.6\) and \(\Lambda=\mathrm{diag}(0.5,0.5)\).
The actuator torque is saturated component-wise as \(u_1\in[-10,10]\ \mathrm{N\cdot m}\), \(u_2\in[-4,4]\ \mathrm{N\cdot m}\).
% Disturbances are injected on both matched and unmatched channels with base magnitudes \([60,150]^\top\), base frequencies \([4,4]^\top\), and seed \(0\) (for reproducibility).
% Numerical safeguards include \(\varepsilon_0=10^{-6}\) in BLF denominators and a fractional floor in \(\rho^2-z^2\), implemented as \(\max\{\rho^2-z^2,\ a_{\mathrm{frac}}\rho^2+\varepsilon_0\}\) with \(a_{\mathrm{frac}}=0.002\), to avoid division by near-zero values.
...
本次实验中，系统初始状态设为 \(x_1(0)=[\pi/2,\pi/2]^{\top}\)，仿真结果如 \cref{fig:2,fig:3,fig:4} 所示. \cref{fig:2}为两关节轨迹 \(q\) 与参考 \(q_d\) 的对比，\cref{fig:3}为跟踪误差 \(z_1\) 及其预设边界 \(\pm\rho(t)\)，\cref{fig:4}为实际控制力矩 \(u\)。\cref{fig:2} 表明轨迹在 \(T_p\) 内快速贴合参考且端点无尖峰；\cref{fig:3} 显示误差全程被 \(\pm\rho(t)\) 约束并在 \(t\approx T_p\) 压缩至稳态约束内，同时可见 \(\rho(t)\) 的动态调整,当输入饱和产生残差 \(\Delta u\) 或扰动增大时， \(\rho(t)\)）暂时放宽，从而在执行器受限时仍保持 \(|e(t)|<\rho(t)\) 的不变性；残差回落后边界自动收紧，误差继续压降。\cref{fig:4} 验证了执行器友好性,初段力矩可能接近限幅，但动态抗饱和补偿 \(\zeta\) 在预定义时间内抽干 \(\Delta u\)，使输入迅速回到线性区。综上表明，所提控制方法在大初值与饱和条件下同时实现了全局性能约束、预定义时间进入与稳态精度，且通过性能函数的在线自适应有效协调了误差收敛速度与执行器负载之间的权衡。
*******************需不需要加扰动、饱和差的仿真结果图**********************

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{fig2.eps}
	\caption{Position tracking performance.}
	\label{fig:2}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Position tracking errors.}
	\label{fig:3}
\end{figure}
...
我们分析了不同初始状态对控制性能的影响，尤其关注在考虑输入饱和与补偿机制下，其收敛性能是否仍然保持一致。初始状态分别设置为：$x_1(0) \in \left\{\left[{\pi}/{2}, {\pi}/{2}\right]^{\!\top}, \left[1, 1\right]^{\!\top},  \left[{-3 \pi}/{4}, {-3 \pi}/{4}\right]^{\!\top}\right\}$.
从图 \cref{fig:5,fig:6,fig:7}可见，对于三组不同初始状态，系统均可以实现快速轨迹跟踪，且跟踪误差处于性能约束的包络内时，误差曲线在 $T_p$ 之前收敛。当饱和加剧时，性能边界会短暂变宽以保障不越界与执行器友好性，补偿收敛后边界自动收紧，最终在不牺牲稳定性的前提下恢复到既定精度。
该实验结果充分验证，无论系统初始状态误差如何，所提出的控制器均能确保在预定义时间 $T_p$ 内实现误差的合法化与全局收敛。这表明系统具备与初始条件无关的时间收敛特性。
...
\begin{figure}[H]
	...
	\caption{Position tracking errors under different initial values of \(x_1(0)x_1(0) = \left[{\pi}/{2}, {\pi}/{2}\right]^{\!\top}\).}
	\label{fig:5}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Position tracking errors under different initial values of \(x_1(0) = \left[1, 1\right]^{\!\top}\).}
	\label{fig:6}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Position tracking errors under different initial values of \(x_1(0) = \left[{-3 \pi}/{4}, {-3 \pi}/{4}\right]^{\!\top}\).}
	\label{fig:7}
\end{figure}
...
The G-PPF adapts online using low-pass filtered residuals \(|\Delta u|,|d|,|e|\) with \((\tau_u,\tau_d,\tau_e)=(0.05,0.1,0.08)\) and gains \((k_u,k_d,k_e)=(0.2,0.15,0.4)\); the pre-phase gain is limited by \(\sigma_0=1.0\), \([\sigma_{\min},\sigma_{\max}]=[1.2,2]\); the post-phase amplitude is capped by \(\Sigma_{\max}=20\) and \(\iota=2\).
The controller parameters are set as: \(\eta=0.1\), \(\beta=0.4\), \(k_1=\mathrm{diag}( 60, 60, 60, 48, 48, 60)\), \(k_2=\mathrm{diag}(12, 10, 12, 10, 8, 8)\).
The dynamic anti-saturation compensator \(\zeta\) adopts \(T_\zeta=1.8\,\mathrm{s}\), \(\gamma=0.4\), \(\mu=14\).
The observer uses \(\omega_d=30\) and \(\Lambda=\mathrm{diag}(60,60,60,60,60,60)\).
The actuator torque is saturated component-wise as $u_{\min}=-u_{\max}, u_{\max}=[100; 100; 40; 30; 20; 15]$.

The initial values of the the 6-DOF manipulator are ${x}(0)=[0, -{\pi}/{6},0, {\pi}/{2},0,{\pi}/{12}]^{\top}$ rad, $\dot{x}(0)=\mathbf{0}$ rad/s. 仿真结果如 \cref{fig:12,fig:13,fig:14} 所示：\cref{fig:12} 表明在强耦合与执行器限幅条件下，各关节均能在预定义时域内快速贴合参考且端点无尖峰；\cref{fig:13} 显示误差全程受 \(\pm\rho_i(t)\) 约束并在进入阶段压缩至小半径，个别关节受限幅或耦合扰动时，性能函数在低通残差驱动下短暂放宽以确保不越界，残差回落后又自动收紧；\cref{fig:14} 进一步表明初段力矩可能贴限，但动态抗饱和补偿 \(\zeta\) 能在短时内抽干饱和残差。综合来看，在 6 自由度强耦合与力矩限幅的现实约束下，GPPF 可依据 \(|\Delta u|,|d|,|e|\) 的低通残差在线自适应调整性能边界，保证误差全程不越界并按预定义时间收敛到小半径，兼顾安全边界与准时到达；在满足工业力矩约束与鲁棒性的前提下，所提方案提供了可调的收敛时限、全程的安全约束与执行器友好性，适合落地于实际多自由度机器人轨迹跟踪控制。


% 面向实际多自由度机械臂的可用性，\cref{fig:12}表明在强耦合与执行器限幅条件下，各关节均能在预定义时域内快速贴合参考且端点无尖峰，体现了 G–PPF 端点 \(C^1\) 平滑与预定义时间可调性的工程可复现；\cref{fig:13}显示误差全程受限并在进入阶段压缩至小半径，个别关节受限幅或耦合扰动时，性能函数通过低通残差驱动的自适应短暂放宽，保障不越界后再自动收紧，\cref{fig:14}进一步表明初段可能贴限，但动态抗饱和补偿 \(\zeta\) 能在短时内抽干饱和残差，使输入回到线性区并在稳态保持低占空、低抖振，有利于电机热管理与寿命。在6自由度强耦合与执行器限幅条件下的仿真表明，该控制算法对实际多自由度机械臂具有直接可用的工程价值，G-PPF 的自适应调整性能边界可依据低通残差在线自适应，确保在电机力矩受限、摩擦/建模误差存在、关节耦合显著时，误差始终不越界并按预定义时间收敛到小半径——这与工业现场既要安全约束、又要准时到达的需求一致；该方法在满足工业力矩约束与鲁棒性的前提下，提供了可调的收敛时限、全程的安全边界与可实现的执行器友好性，适合落地到实际多自由度机器人轨迹跟踪控制。

% 6 自由度机械臂的初始条件设为 ${x}(0)=[0,\,-{\pi}/{6},\,0,\,{\pi}/{2},\,0,\,{\pi}/{12}]^{\top}\ \mathrm{rad}$、$\dot{x}(0)=\mathbf{0}\ \mathrm{rad/s}$，
...
\begin{figure}[H]
	...
	\caption{Position tracking trajectories of 6-DOF manipulator.}
	\label{fig:12}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Position tracking errors of 6-DOF manipulator.}
	\label{fig:13}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Control input torques of 6-DOF manipulator.}
	\label{fig:14}
\end{figure}
