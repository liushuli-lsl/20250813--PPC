...
\Phi_{i}=\frac{z_{1,i}\left(\rho_{1,i}^{2}-z_{1,i}^{2}\right)}{\rho_{1,i}^{4}}
...
-\mathcal{K}_{1,i}(z_{1,i},\rho_{1,i})\Phi_{i}
-k_{1,i} \Phi_{i},
\end{equation}
...
% &+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-\frac{P_1}{P_2}z_1+\tfrac{z_{2}^{3}}{\rho_{2}^{3}}\dot\rho_{2}-\mathcal{K}_{2}(z_{2},\rho_{2}) \Phi_{2}
% -\hat \omega
...
% u =&C(q,\dot{q})x_2 + G(q)\\
% &+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-\frac{P_1}{P_2}z_1+\tfrac{z_{2}^{3}}{\rho_{2,i}^{3}}\dot\rho_{2,i}-\mathcal{K}_{2}(z_{2},\rho_{2,i}) \Phi_{2}
% -\hat{h}(\chi)
% -\hat \omega
...
The prescribed–performance horizon and shaping are \(T_p=3\,\mathrm{s}\), \(p=0.3\), with steady tube floor \(a=0.01\).
...
The controller parameters are set as: \(\eta=0.1\), \(\beta=0.2\), \(k_1=\mathrm{diag}(36,30)\), \(k_2=\mathrm{diag}(12,12)\), \(k_s=2.2\).
