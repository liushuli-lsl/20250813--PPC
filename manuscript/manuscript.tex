...
$$
\frac{\rho_{j,i}^4 z_{j,i}}{(\rho_{j,i}^2-z_{j,i}^2)^2}* \\
\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}
$$

\begin{equation}\label{eq:25}
\alpha_i = \dot q_{d,i}-\zeta_i
-\mathcal{K}_{1,i}(z_{1,i},\rho_{1,i})\Phi_{1,i}
+\frac{z_{1,i}^3}{\rho_{1,i}^3}\,\dot\rho_{1,i}-k_{1,i}z_{1,i},
\end{equation}
...
Let $Q=\mathrm{col}\{Q_{2,i}\}$,
$P=\mathrm{col}\{P_{2,i}\}$,
$\Phi_2=\mathrm{col}\{\Phi_{2,i}\}$. The control input torque is designed as
\begin{equation}\label{eq:tau-cmd}
\begin{aligned}
u =&C(q,\dot{q})x_2 + G(q)\\
&+M(q)\left[-f(x)+\dot\alpha^{f}+\delta -\mu_2\,\zeta-\frac{Q}{P}z_2+\tfrac{z_{2}^{3}}{\rho_{2,i}^{3}}\dot\rho_{2,i}
-\hat{h}(\chi)
-\hat w
-k_s\,\mathrm{sgn}(z_{2})-\mathcal{K}_{2}(z_{2},\rho_{2,i}) \Phi_{2}-k_{2}z_{2}\right]
\end{aligned}
\end{equation}
...
\dot\theta_i = \varrho_i\,P_{2,i}\,z_{2,i}\,\psi(\chi) - \kappa_i\,\theta_i,\qquad
\varrho_i,\kappa_i>0.
\end{equation}
...
+\sum_{i=1}^n \left( -P_{2,i} k_{2,i}z_{2,i}-\Psi(V_{2,i})-P_{2,i}\tilde{w}_i  \right)-k_s\sum_{i=1}^n |P_{2,i}|\left\lVert  \mathrm{sgn}(z_{2,i})  \right\rVert_1
\end{equation}
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		V= &\sum_{i=1}^n(V_{1,i})+\tfrac{1}{2}\tilde\alpha^\top \tilde\alpha+\tfrac{1}{2\gamma_o}\tilde w^\top\tilde w+\sum_{i=1}^n\tfrac{1}{2\varrho_i}\|\tilde\theta_i\|^2\\
		=&\sum_{i=1}^n \left( -P_{1,i} k_{1,i}z_{1,i}-\Psi(V_{1,i})-P_{1,i}\tilde\alpha_i\right)
+\sum_{i=1}^n \left( -P_{2,i} k_{2,i}z_{2,i}-\Psi(V_{2,i})-P_{2,i}\tilde{w}_i  \right)\\
&-k_s\sum_{i=1}^n |P_{2,i}|\left\lVert  \mathrm{sgn}(z_{2,i})  \right\rVert_1+\tfrac{1}{2}\tilde\alpha^\top \tilde\alpha+\tfrac{1}{2\gamma_o}\tilde w^\top\tilde w+\sum_{i=1}^n\tfrac{1}{2\varrho_i}\|\tilde\theta_i\|^2
	\end{aligned}
\end{equation}
				 ...
				 -\sum_{i}
					...
					\left(
						-\frac{\beta}{\omega_{2}}\varrho_i
						 \varGamma_{2,i}\psi_i
						+\kappa_i\hat \theta_{i}
					\right)                                                    = -\sum_{i=1}^{n}\frac{\kappa_i}{\varrho_i}\tilde \theta_{i}^{\top}\hat \theta_{i}       \\
...
% - \frac{1}{\omega_1} \frac{z_{1,i} \hat{d}_{1,i}}{\rho_{1,i}^2 - z_{1,i}^2}\\
...
% + \frac{z_{1,i} \tilde{d}_{1,i}}{\omega_1(\rho_{1,i}^2 - z_{1,i}^2)}
% - \frac{z_{1,i} d_{1,i}}{\omega_1(\rho_{1,i}^2 - z_{1,i}^2)}\\
...
% &+\frac{1}{\omega_1} \frac{z_{1,i}^2}{\rho_{1,i}^2 - z_{1,i}^2}+\frac{1}{2\omega_1  (\rho_{1,i}^2 - z_{1,i}^2)} \tilde{d}_{1,i}^2+\frac{1}{2\omega_1  (\rho_{1,i}^2 - z_{1,i}^2)} {d}_{1,i}^2\\
% &\le \left(\frac{1}{2\hbar_{1,i}}+ \frac{1}{2}-\frac{\varpi_{1,i}}{2 \hbar_{1,i}}+ \frac{1}{2\omega_1  (\rho_{1,i}^2 - z_{1,i}^2)}\right)\tilde{d}_{1,i}^2+ \frac{ 1}{2\hbar_{1,i}} \dot{\bar{{d}}}_{1,i}^2+\frac{\varpi_{1,i}}{2 \hbar_{1,i}}{{d}^2_{1,i}}+\frac{1}{\omega_1} \frac{z_{1,i}^2}{\rho_{1,i}^2 - z_{1,i}^2}+\frac{1}{2\omega_1  (\rho_{1,i}^2 - z_{1,i}^2)} {d}_{1,i}^2
% \end{aligned}
% \end{equation}
