...
\begin{equation}\label{eq:22}
	...
	\alpha^{f}(0)=\alpha(0),
\end{equation}
where $0<\lambda<1$. Define the filtering error is
...
记
$
G_i=\frac{\rho_{1}^4 z_{1,i}}{(\rho_{1}^2-z_{1,i}^2)^2},
H_i=\frac{\rho_{1} z_{1,i}^4}{(\rho_{1}^2-z_{1,i}^2)^2},
\Phi_{1,i}=\frac{z_{1,i}\bigl(\rho_{1}^{2}-z_{1,i}^{2}\bigr)}{\rho_{1}^{4}}
$,
$\dot z_{1,i}= z_{2,i}-\dot q_{d,i}+\alpha^{f}_i+\zeta_i$ 代入 BLF 导数后，并根据公式（），得到

$$
\dot V_1=\sum_i\!\Big[G_i z_{2,i}+G_i(\alpha_i+\zeta_i-\dot q_{d,i})-G_i\tilde\alpha_i-H_i\dot\rho_{1}\Big]
$$

Let $\Psi(V_j)=\frac{\pi}{ \eta T_p}\Big(V_j^{\,1-\eta/2}+n^{-\frac{n}{2} }V_j^{\,1+\eta/2}\Big), j\in\{1,2\}$, 我们设计
$$
\mathcal{K}_{1,i}(z_{1,i},\rho_{1})
=\frac{\rho_{1}^2}{2}\frac{\Psi(V_1,i)}{V_1,i}
...
\(
P_i = \frac{\rho_{2}^4 z_{2,i}}{\bigl(\rho_{2}^{2}-z_{2,i}^{2}\bigr)^{2}},
Q_{i}=\frac{\rho_{2} z_{2,i}^{4}}{\bigl(\rho_{2}^{2}-z_{2,i}^{2}\bigr)^{2}},
\Phi_{2,i}=\frac{z_{2,i}\bigl(\rho_{2}^{2}-z_{2,i}^{2}\bigr)}{\rho_{2}^{4}}.
\), and using $\dot z_{2,i}=\dot x_{2,i}-\dot\alpha_i^{f}-\dot\zeta_i$, we obtain the time derivative of $V_2$ is as
...
=\frac{\pi\,\rho_{2}^2}{2\eta T_p}\Big((V_{2,i})^{-\eta/2}+n^{-\frac{n}{2} }(V_{2,i})^{\eta/2}\Big).
\end{equation}
...
where $k_{2,i}>0$, $k_s>0$.

Let $\chi={q,\dot{q}}\in\mathbb{R}^m$ be the regressor and $\xi(\chi)\in\mathbb{R}^{N}$ the normalized basis. Approximate the structured uncertainty by 
...
\end{cases}
\end{equation}
where $\Lambda_o=\mathrm{diag}\{\lambda_{o,i}\}>0, \vartheta=x_2(0)$,
设一阶跟踪微分器估计误差项为 $\tilde w=\hat w-w$.
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		V= \sum_i(V_{1,i}+V_{2,i})+\tfrac{1}{2}\tilde\alpha^\top \tilde\alpha+\tfrac{1}{2\gamma_o}\tilde w^\top\tilde w+\sum_i\tfrac{1}{2\varrho_i}\|\tilde\theta_i\|^2.
	\end{aligned}
\end{equation}
