...
\right]\\
&=\sum_{i=1}^{n}\left[
\frac{\rho_{1}^4\,z_{1,i}}{\big(\rho_{1}^{2}-z_{1,i}^{2}\big)^{2}}(e_{2,i}-\dot q_{d,i}+\alpha^{f}_i+\zeta_i)
\;-\;
\frac{\rho_{1}\,z_{1,i}^{4}}{\big(\rho_{1}^{2}-z_{1,i}^{2}\big)^{2}}\;\dot \rho_{1}
\right]
\end{aligned}
...
记
$$
S_{1,i} = 2V_{1,i}=\frac{\rho_{1}^2 z_{1,i}^2}{\rho_{1}^2-z_{1,i}^2},\qquad
...
选用标准 PTS 速度函数：$
\Psi(S)=\frac{\pi}{\eta T_p}\Big(S^{\,1-\eta/2}+S^{\,1+\eta/2}\Big).
$

$$
\mathcal{K}_{1,i}(z_{1,i},\rho_{1})
=\rho_{1}^2\;\frac{\Psi_1(S_i)}{S_i}
=\frac{\pi\,\rho_{1}^2}{\eta T_p}\Big((2V_{1,i})^{-\eta/2}+(2V_{1,i})^{\eta/2}\Big).
$$
...
-\;\mathcal{K}_{1,i}(z_{1,i},\rho_{1})\,
\frac{z_{1,i}(\rho_{1}^2-z_{1,i}^2)}{\rho_{1}^4}
...
\dot V_1
= \sum_{i=1}^n \Big[
-G_i k_{1,i}z_{1,i}\;-\;\Psi_1(2V_{1,i})\;-\;G_i\tilde\alpha_i
\Big].
$$
...
z_{2,i}=z_{2,i}=x_{2,i}-\alpha_i^{f}-\zeta_i .
\]
...
\[
\dot x_2 = f(x)+\phi(x)+g(q)\,u_{\rm act}+d'(t),
\]
...
\[
\dot V_2=\sum_{i=1}^n \Big[\,P_i\,\dot z_{2,i}-Q_{i}\,\dot\rho_{2}\,\Big].
\]

Define $S_{2,i}= 2V_{2,i}=\frac{\rho_{2}^2 z_{2,i}^2}{\rho_{2}^2-z_{2,i}^2}$. Choose the state-dependent gain
\begin{equation}\label{eq:k2-pts}
\mathcal{K}_{2,i}(z_{2,i},\rho_{2})=\rho_{2}^{2}\,\frac{\Psi(S_{2,i})}{S_{2,i}}
=\frac{\pi\,\rho_{2}^2}{2\eta T_p}\Big((2V_{2,i})^{-\eta/2}+(2V_{2,i})^{\eta/2}\Big).
\end{equation}
...
\dot\theta_i = \kappa_i\,P_i\,z_{2,i}\,\xi(\chi) \;-\; \varrho_i\,\theta_i,\qquad
\kappa_i>0,\ \varrho_i \ge 0.
\end{equation}
...
\]
the Disturbance observer (DOB) estimating the residual disturbance is
\begin{equation}\label{eq:dob}
\dot{\hat d}= -\Lambda_o\,\hat d
+\Lambda_o\Big(\hat{\dot x}_2 - f(x) - g(q)\,u_{\rm act} - \hat\phi(\chi)\Big),
\quad \Lambda_o=\mathrm{diag}\{\lambda_{o,i}\}>0,
\end{equation}
where $u_{\rm act}$ uses the saturated torque $\tau_{\rm act}$.
...
\;-\;\mathrm{diag}(Q./P)\,z_{2}
\;-\;\mathcal{K}_{2}(z_{2},\rho_{2})\odot \Phi_{2}
...
P_i\!\left(\tfrac{z_{2,i}^3}{\rho_{2}^3}\dot\rho_{2}\right)=H_{2,i}\dot\rho_{2},
\]
so that
\[
\dot V_1+\dot V_2
...
% % \begin{equation}
% % 	\dot{V}_{1} = \sum_{i=1}^{n} \left[ \frac{z_{1,i} \dot{z}_{1,i}}{\rho_{1}^{2} - z_{1,i}^{2}} - \frac{\dot{\rho}_{1,i}}{\rho_{1}} \frac{z_{1,i}^{2}}{\rho_{1}^{2} - z_{1,i}^{2}} \right]
% % 	\label{eq:V1}
% % 	\end{equation}

% 	\begin{equation}\label{eq:27}
% 	\dot{V}_{1}=-\frac{\dot{\omega}_{1}}{2 \omega_{1}^{2}}\sum_{i=1}^{n}\ln \frac{\rho_{1}^{2}}{\rho_{1}^{2}-z_{1,i}^{2}}+\frac{1}{\omega_{1}} \sum_{i=1}^{n} \left[ \frac{z_{1,i} \dot{z}_{1,i}}{\rho_{1}^{2} - z_{1,i}^{2}} - \frac{\dot{\rho}_{1,i}}{\rho_{1}} \frac{z_{1,i}^{2}}{\rho_{1}^{2} - z_{1,i}^{2}} \right].
% 	\end{equation}

% 	According to \cref{eq:19,eq:20}, one has
% 	\begin{equation}	\label{eq:28}
% 	\dot{z}_{1,i} = \dot{\beta} e_{1,i} + \beta \dot{e}_{1,i}=\dot{\beta} e_{1,i} + \beta (e_{2,i}-\dot q_{i,d}+\alpha_{i}-\delta_{1,i}+\mu_1 \zeta_{1,i}),
% 	\end{equation}

% 	Substituting \cref{eq:28} into \cref{eq:27}, and letting $\varGamma_{1,i} = {z_{1,i}}/({\rho_{1}^2 - z_{1,i}^2})$, we have
% 	\begin{equation}	\label{eq:29}
% 			\begin{aligned}
% 			\dot{V}_{1} &= -\frac{\dot{\omega}_{1}}{2 \omega_{1}^{2}}\sum_{i=1}^{n}\ln\frac{\rho_{1}^{2}}{\rho_{1}^{2}-z_{1,i}^{2}}+\frac{1}{\omega_{1}}\sum_{i=1}^{n} \left[ \frac{z_{1,i} (\dot{\beta} e_{1,i} + \beta \dot{e}_{1,i})}{\rho_{1}^{2} - z_{1,i}^{2}} - \frac{\dot{\rho}_{1,i}}{\rho_{1}} \frac{z_{1,i}^{2}}{\rho_{1}^{2} - z_{1,i}^{2}} \right] \\
% 			% &=-\frac{\dot{\omega}_{1}}{2 \omega_{1}^{2}}\sum_{i=1}^{n}\frac{z_{1,i}^{2}}{\rho_{1}^{2}-z_{1,i}^{2}}+\frac{1}{\omega_{1}}\sum_{i=1}^{n} \frac{\dot{\beta} e_{1,i} z_{1,i}}{\rho_{1}^{2} - z_{1,i}^{2}} +\frac{1}{\omega_{1}}\sum_{i=1}^{n} \varGamma_{1,i} \left[\beta \left(e_{2,i}-\dot q_{i,d}+\alpha_{i}-\delta_{1,i}+\mu_1 \zeta_{1,i}- \frac{\dot{\rho}_{1,i}e_{1,i}}{\rho_{1}}\right)  \right]\\
% 			&=-\frac{\dot{\omega}_{1}}{2 \omega_{1}^{2}}\sum_{i=1}^{n}\frac{z_{1,i}^{2}}{\rho_{1}^{2}-z_{1,i}^{2}}+\frac{\varGamma_{1,i}}{\omega_{1}}\sum_{i=1}^{n} \left[\dot{\beta} e_{1,i}+\beta \left(e_{2,i}-\dot q_{i,d}+\alpha_{i}-\delta_{1,i}+\mu_1 \zeta_{1,i}- \frac{\dot{\rho}_{1,i}}{\rho_{1}}e_{1,i}\right)  \right],
% 		\end{aligned}
% 			\end{equation}
...
% 	\begin{aligned}
% 	\alpha_i
% 	% &= \dot q_{i,d}-e_{2,i}+\delta_{1,i}-\mu_1 \zeta_{1,i}+\frac{\dot{\omega}_{1}e_{1,i}}{2 \omega_{1}}-\frac{\iota_1  \dot{\beta}^{2} e_{1,i}^{3} }{\rho_{1}^{2} - z_{1,i}^{2}} 
% 	% +\frac{\dot\rho_{1}}{\rho_{1}} e_{1,i}- k_{1,i} e_{1,i}
% 	% - \omega_{1} e_{1,i} \Psi_{1,i}-{\hat d}_{1,i}\\
% 	&= \dot q_{i,d}-e_{2,i}+\delta_{1,i}-\mu_1 \zeta_{1,i}+e_{1,i}\left( \frac{\dot{\omega}_{1}}{2 \omega_{1}}-\frac{\iota_1  \dot{\beta}^{2} e_{1,i}^{2} }{\rho_{1}^{2} - z_{1,i}^{2}} 
% 	+\frac{\dot\rho_{1}}{\rho_{1}} - k_{1,i} 
% 	- \omega_{1} \Psi_{1,i}   \right)-{\hat d}_{1,i}
% \end{aligned}
% \end{equation}
% where, $k_{1}=\text{diag}(k_{1,1}, k_{2,1},\cdots, k_{n,1})$, $k_{1,i}>0 $, $\iota_2$, $\Psi_{1,i}={\pi}/({2 \eta\,T_p})[1+(n z_{1,i}\varGamma_{1,i}/2)^{{\eta}/{2}}]$ is a regulation term associated with a predefined-time and performance function to achieve convergence regulation of the error. \(\hat d_{1,i}\) is a disturbance estimate.
% Introduce the composite error and the first-order sliding disturbance observer:
% \begin{equation}\label{eq:31}
% 	 s_{1,i} = e_{2,i} + v_{1,i} e_{1,i}, \quad v_{1,i}>0,
% \end{equation}
% % 其中$v_{1,i}>0$。
...
% % 为实现扰动估计，引入辅助信号$\xi_{1,i}$, \(\hat d_{1,i}\)的动态方程如下
% %  \begin{equation}\label{eq:32}
% %       \dot{\hat d}_{1,i} = -\hbar_{1,i} |s_{1,i} \vert^{\frac{1}{2}} \operatorname{sgn}(s_{1,i})+\xi_{1,i}, 
% % \end{equation}
% % \begin{equation}\label{eq:32}
% %  \dot{\xi}_{1,i} = -\hbar_{2,i} \operatorname{sgn}(s_{1,i}),
% % \end{equation}
% % 其中$\hbar_{1,i} , \hbar_{2,i} >\bar{d}$. 定义估计误差为$\tilde{d}_{1,i} = d_{1,i} - \hat{d}_{1,i}$, $\dot{\tilde{d}}_{1,i} = \hbar_{1,i}\left| s_{1,i} \right|^{\frac{1}{2}} \operatorname{sgn}(s_{1,i}) - \xi_{1,i} + \dot{d}_{i.1}$.
...
% % 将扰动估计量\(\hat d_i\)表示为
% %  \begin{equation}\label{eq:32}
% %       \dot{\hat d}_{1,i} = -\hbar_{1,i} \operatorname{sat}(s_i).
% % \end{equation}
% %   其中$\hbar_{1,i} >\bar{d}$.
 
% % \(\hat d_{1,i} \)的动态方程设计如下
% \begin{equation}\label{eq:32}
%       \dot{\hat d}_{1,i} = -\hbar_{1,i} \operatorname{sgn}(s_{1,i})-\varpi_{1,i} {\hat d}_{1,i}+\hbar_{1,i}\frac{\varGamma_{1,i}}{{\omega}_{1}},
% \end{equation}
% where $\hbar_{1,i} >\bar{d}, \varpi_{1,i}>\hbar_{1,i}+1$, and $\tilde{d}_{1,i} = d_{1,i} - \hat{d}_{1,i}$.
...
% 		\begin{equation}\label{eq:33}
% -\frac{\dot{\omega}_{j}}{2 \omega_{j}^{2}}\sum_{i=1}^{n}\ln\frac{\rho_{j,i}^{2}}{\rho_{j,i}^{2}-z_{j,i}^{2}}
% \leq -\frac{\dot{\omega}_{j}}{2 \omega_{j}^{2}}\sum_{i=1}^{n}\frac{z_{j,i}^{2}}{\rho_{j,i}^{2}-z_{j,i}^{2}},
% 	\end{equation}
% 		\begin{equation}\label{eq:34}
% 			\sum_{i=1}^{n} \frac{\dot{\beta} e_{j,i} z_{j,i}}{\rho_{j,i}^{2} - z_{j,i}^{2}} 
% 			\leq 
% 			 \sum_{i=1}^{n} \frac{\iota_j  \dot{\beta}^{2} e_{j,i}^{2} z_{j,i}^{2}}{(\rho_{j,i}^{2} - z_{j,i}^{2})^{2}} 
% 			+ \frac{n}{4\iota_j}.
% 			\end{equation}
			
% % From \cref{lemma:1}, letting $\mathcal{X} =1/2\sum_{i=1}^{n} \ln({\rho_{j}^2}/{\rho_{j}^2-e_{j,i}^2}), \mathcal{Y} =1, \mathcal{Z}_{1}= {1-{\eta}/{2}}, \mathcal{Z}_{2}={\eta}/{2}, \mathcal{Z}_{3}= (\eta/2)^{(\eta/(2-\eta))}$, we have
...
% % 	\begin{aligned}
% % 	\dot{V}_{1} 
% % 	&\le\sum_{i=1}^{n} \varGamma_{1,i} \frac{1}{\omega_{1}}\left[-\frac{\dot{\omega}_{1}z_{1,i}}{2 \omega_{1}}+\frac{\iota_1  \dot{\beta}^{2} e_{1,i}^{2} z_{1,i}}{\rho_{1}^{2} - z_{1,i}^{2}}  + \beta  \left(e_{2,i}-\dot q_{i,d}+\alpha_{i}-\delta_{1,i}-\mu_1 \zeta_{1,i}- \frac{\dot{\rho}_{1,i} e_{1,i}}{\rho_{1}}\right)  \right] + \frac{1}{4 \omega_{1} \iota_1 }
% % 	\\
% % 	&\le\sum_{i=1}^{n} \varGamma_{1,i} \left[ \frac{\beta}{\omega_{1}} \left(-\frac{\dot{\omega}_{1}e_{1,i}}{2 \omega_{1}}+\frac{\iota_1  \dot{\beta}^{2} e_{1,i}^{3}}{\rho_{1}^{2} - z_{1,i}^{2}}+e_{2,i}-\dot q_{i,d}+\alpha_{i}-\delta_{1,i}-\mu_1 \zeta_{1,i}- \frac{\dot{\rho}_{1,i} e_{1,i}}{\rho_{1}}\right) \right] + \frac{1}{4 \omega_{1} \iota_1 },
% % \end{aligned}
% % 	\end{equation}
...
% 	\begin{aligned}
% 		\dot{V}_{1}
% 		 & \le - \sum_{i=1}^{n}\varGamma_{1,i}z_{1,i}\Psi_{1,i}  - \frac{1}{\omega_{1}}\sum_{i=1}^{n} \left(\frac{k_{1,i} z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}-  \frac{ z_{1,i}\hat d_{1,i}}{\rho_{1}^2 - z_{1,i}^2}\right) + \frac{n}{4  \omega_{1} \iota_1 }.                           
% 	\end{aligned}
% \end{equation}
% % \begin{equation}\label{eq:23}
% % 	\begin{aligned}
% % 		\dot{V}_{1}
% % 		 & = - \sum_{i=1}^{n}\varGamma_{1,i}z_{1,i}\left[\frac{\pi}{\eta\,T_p}\,
% % 	\left( 2^{-1}
% % 	+
% % 	n^{\frac\eta2} 2^{-\tfrac\eta2 -1}\,\left(\frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}\right)^{\frac{\eta}{2}}\right)  \right]                             \\
% % 		 & -\frac{\beta}{\omega}\sum_{i=1}^{n}\frac{ z_{1,i}{\tilde{\alpha}}}{\rho_{1}^{2}-z_{1,i}^{2}}- \sum_{i=1}^{n}k_{1,i} \frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}- \sum_{i=1}^{n}k_{i,\sigma} \frac{ z_{1,i}\sigma_{1,i}}{\rho_{1}^2 - z_{1,i}^2}+ \frac{1}{4  \omega \iota_1 } ,                            
% % 	\end{aligned}
% % \end{equation}
...
% 	V_2=V_1+\frac{1}{2 \omega_2}\sum_{i=1}^{n} \ln\frac{\rho_{2}^2}{\rho_{2}^2-z_{2,i}^2}, 
% \end{equation}

% The time derivation of $V_2$ yields
% \begin{equation}\label{eq:37}
% 	\dot{V}_{2} = \dot{V}_{1}-\frac{\dot{\omega}_{2}}{2 \omega_{2}^{2}}\sum_{i=1}^{n}\ln\left(\frac{\rho_{2}^{2}}{\rho_{2}^{2}-z_{2,i}^{2}}\right)+\frac{1}{\omega_{2}}\sum_{i=1}^{n} \left[ \frac{z_{2,i} \dot{z}_{2,i}}{\rho_{2}^{2} - z_{2,i}^{2}} - \frac{\dot{\rho}_{2,i}}{\rho_{2}} \frac{z_{2,i}^{2}}{\rho_{2}^{2} - z_{2,i}^{2}} \right].
% 	\end{equation}
	
% From \cref{eq:2,eq:19,eq:20}, one has 
% 	\begin{equation}\label{eq:38}
% 		\begin{aligned}
% 	\dot{z}_{2,i} =& \dot{\beta} e_{2,i} + \beta \dot{e}_{2,i}
% 	 =\dot{\beta} e_{2,i} + \beta \left(f_i + g_i u_i + h_i-\dot{\alpha}^{f}_{i}-\delta_{2,i}+\mu_2 \zeta_{2,i}\right).
% \end{aligned}
% 	\end{equation}

% Substituting \cref{eq:38} into \cref{eq:37}, and letting $\varGamma_{{2,i}}  ={z_{2,i}}/({\rho_{{2,i}}^2 - z_{2,i}^2})$ gives
% \begin{equation}\label{eq:39}
% 	\begin{aligned}
% 		\dot V_2
% 		%  & =
% 		% \dot{V}_{1} -\frac{\dot{\omega}_{2}}{2 \omega_{2}^{2}}\sum_{i=1}^{n}\ln\left(\frac{\rho_{2}^{2}}{\rho_{2}^{2}-z_{2,i}^{2}}\right)+\frac{1}{\omega_{2}}\sum_{i=1}^n \varGamma_{{2,i}} \left[\dot{\beta} e_{2,i} + \beta \left(f_i + g_i u_i + h_i-\dot{\alpha}^{f}_{i}-\delta_{2,i}+\mu_2 \zeta_{2,i}-\frac{e_{2,i}\dot{\rho}_{2,i}}{\rho_{{2,i}}}\right)\right]\\
% 		%  & =\dot{V}_{1}+ \sum_{i=1}^n \varGamma_{{2,i}} \frac{\beta}{\omega_{2}} \left(-\frac{\dot{\omega}_{2} e_{2,i}}{2 \omega_{2}}+f_i + g_i u_i + h_i-\dot{\alpha}^{f}_{i}-\delta_{2,i} +\mu_2 \zeta_{2,i} +\frac{\iota_2  \dot{\beta}^{2} e_{2,i}^{3}}{\rho_{1}^{2} - e_{2,i}^{2}}-\frac{e_{2,i}\dot{\rho}_{2,i}}{\rho_{{2,i}}} \right)+\frac{1}{4 \omega_{2}\iota_2}\\
% 		 & =\dot{V}_{1}+ \sum_{i=1}^n \frac{\beta}{\omega_{2}} \varGamma_{{2,i}}  \left[f_i + g_i u_i + h_i-\dot{\alpha}^{f}_{i}-\delta_{2,i} +\mu_2 \zeta_{2,i} +e_{2,i}\left(\frac{\iota_2  \dot{\beta}^{2} e_{2,i}^{2}}{\rho_{1}^{2} - z_{2,i}^{2}} -\frac{\dot{\omega}_{2}}{2 \omega_{2}}  -\frac{\dot{\rho}_{2,i}}{\rho_{{2,i}}}\right) \right]+\frac{n}{4 \omega_{2} \iota_2},
% 	\end{aligned}
% \end{equation}
...
% 	u_i = g_i^{-1}\left[-\,f_i - \hat{h}_i + \dot{\alpha}^{f}_{i}+\delta_{2,i}-\mu_2 \zeta_{2,i}+e_{2,i}\left(-\frac{\iota_2  \dot{\beta}^{2} e_{2,i}^{2}}{\rho_{1}^{2} - z_{2,i}^{2}} +\frac{\dot{\omega}_{2}}{2 \omega_{2}}+\frac{\dot{\rho}_{2,i}}{\rho_{{2,i}}}- k_{2,i}- \omega_{2}\Psi_{{2,i}}   \right)-\hat d_{2,i}
% 	\right],
% \end{equation}
% % \begin{equation}\label{eq:26}
% % 	u = Cx_2 + G + M \left[\,
...
% % 	\right],
% % \end{equation}
% where $k_{2}=\text{diag}(k_{1,2}, k_{2,2},\cdots, k_{n,2})$, $k_{2,i}>0 $, $\iota_2$, $ \Psi_{2,i}={\pi}/({2 \eta\,T_p})
% 	[1+(n z_{2,i}\varGamma_{2,i}/2)^{{\eta}/{2}}]$.
	
% The unknown dynamics is approximated online by an RBFNN,
% $\hat h_i=\hat w_i^{\top}Q_i(\chi)$ with input $\chi=[x_{1,i},x_{2,i}]^\top$.
% The weight adaptation is chosen as
% \begin{equation}\label{eq:42}
% 	\dot{\hat w}_i
% 	=\frac{\beta }{\omega_{2}} \varrho_i\, Q_i\, \varGamma_{{2,i}} - \kappa_i \,\hat w_i,
% \end{equation}
% with learning rate $\varrho_i>0$ and leakage $\kappa_i>0$.

% For disturbance rejection, define the composite sliding signal
% $s_{2,i}=e_{2,i}+v_{2,i}e_{1,i}$ ($v_{2,i}>0$) and employ
% \begin{equation}\label{eq:41}
% 	\dot{\hat d}_{2,i} = -\hbar_{2,i} \operatorname{sgn}(s_{2,i})-\varpi_{2,i} {\hat d}_{2,i}+\hbar_{2,i} \frac{ \varGamma_{{2,i}}}{{\omega}_{1}},
% \end{equation}
% where $\hbar_{2,i} >\bar{d}, \varpi_{2,i}>\hbar_{2,i}+1$, and $\tilde{d}_{2,i} = d_{2,i} - \hat{d}_{2,i}$.
% % 构造滑模面如下：
% % \begin{equation}\label{eq:41}
% % 	 s_{2,i} = y_{2,i} + v_{2,i} y_{1,i}, 
% % \end{equation}
% % 其中$y_{1,i}, y_{2,i}$定义如下：
% % \begin{equation}\label{eq:46}
% % y_{1,i}=\dot q_i -\alpha_{i,\mathrm{sat}},
% % \quad
% % y_{2,i}=\ddot q_i-\dot{\tilde\alpha}_i.
% % \end{equation}


%   % 扰动估计量\(\hat d_{2,i}\)表示为
% %  \begin{equation}\label{eq:47}
% %       \dot{\hat d}_{2,i} = -k_{d,2}\operatorname{sgn}(s_{2,i}).
% % \end{equation}
% % 其中$k_{d,2}>\bar{d}$.
% % 为实现扰动估计，引入辅助信号$\xi_{2,i} $, \(\hat d_{2,i} \)的动态方程如下
% %  \begin{equation}\label{eq:32}
% %       \dot{\hat d}_{2,i}  = -\varpi_{1,i}  |s_{2,i}  \vert^{\frac{1}{2}} \operatorname{sgn}(s_{2,i} )+\xi_{2,i} , 
% % \end{equation}
% % \begin{equation}\label{eq:32}
% %  \dot{\xi}_{2,i}  = -\varpi_{2,i} \operatorname{sgn}(s_{2,i} ),
% % \end{equation}
% % 其中$\varpi_{1,i}, \varpi_{2,i} >\bar{d}$. 定义估计误差为$\tilde{d}_{2,i}  = d_{2,i}  - \hat{d}_{2,i} $, $\dot{\tilde{d}}_{2,i}  = \varpi_{1,i}\left| s_{2,i}  \right|^{\frac{1}{2}} \operatorname{sgn}(s_{2,i} ) - \xi_{2,i} + \dot{d}_{2,i}$.


% % 令$s_{i} = \frac{e_{{2,i}}}{\rho_{{2,i}}^2 - e_{{2,i}}^2}$


% % According to Young's inequality, we have
% % \begin{equation}\label{eq:43}
% % 		\sum_{i=1}^{n} \frac{\dot{\beta} e_{2,i} z_{2,i}}{\rho_{1}^{2} - z_{2,i}^{2}} 
% % 		\leq 
% % 		 \sum_{i=1}^{n} \frac{\iota_2  \dot{\beta}^{2} e_{2,i}^{2} z_{2,i}^{2}}{(\rho_{1}^{2} - z_{2,i}^{2})^{2}} 
% % 		+ \frac{n}{4\iota_2 }.
% % \end{equation}
...
% % 	\begin{aligned}
% % 		\dot V_2
% % 		 & \le \dot{V}_{1}+ \sum_{i=1}^n \varGamma_{{2,i}} \frac{\beta}{\omega_{2}} \left(-\frac{\dot{\omega}_{2} e_{2,i}}{2 \omega_{2}}+f_i + g_i u_i + h_i+\dot{\alpha}^{f}_{i}-\delta_{2,i} -\mu_2 \zeta_{2,i} +\frac{\iota_2  \dot{\beta}^{2} e_{2,i}^{3}}{\rho_{1}^{2} - e_{2,i}^{2}}-\frac{e_{2,i}\dot{\rho}_{2,i}}{\rho_{{2,i}}} \right)+\frac{1}{4\iota_2},
% % 	\end{aligned}
% % \end{equation}
...
% 	\begin{aligned}
% 		\dot{V}_{2}
% 		& = - \sum_{i=1}^{n}\varGamma_{1,i}z_{1,i}\Psi_{1,i}  - \frac{1}{\omega_{1}}\sum_{i=1}^{n}\frac{k_{1,i} z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}- \frac{1}{\omega_{1}}\sum_{i=1}^{n} \frac{ z_{1,i}\hat d_{1,i}}{\rho_{1}^2 - z_{1,i}^2} \\
% 		 & - \sum_{i=1}^{n}\varGamma_{2,i}z_{2,i}\Psi_{2,i} - \frac{1}{\omega_{2}}\sum_{i=1}^{n}\frac{ k_{2,i} z_{2,i}^2}{\rho_{2i}^2 - e_{2,i}^2}  + \frac{\beta }{\omega_{2}}\sum_{i=1}^{n} \varGamma_{2,i} (h_{i}-\hat{h}_{i})  \\
% 		 &- \frac{1}{\omega_{2}}\sum_{i=1}^{n} \frac{ z_{2,i}\hat d_{2,i}}{\rho_{2}^2 - z_{2,i}^2}+ \frac{n}{4  \omega_{1} \iota_1 } + \frac{n}{4  \omega_{2}\iota_2 }  .                           
% 	\end{aligned}
% \end{equation}
...
% % 	\begin{aligned}
% % 		\dot{V}_{2}
% % 		 & =-\sum_{i=1}^{n}\varGamma_{1,i}\left({\tilde{\alpha}}+ k_{1,i} e_{1,i}+ k_{i,\sigma} \sigma_{i}+ e_{1,i} \Psi_{1,i}\right)
% % 		- \sum_{i=1}^n \varGamma_{2,i}\left( k_{2i} e_{2,i}+e_{2,i} \Psi_{2i}  -\frac{e_{2,i}\dot{\rho_{2i}}}{\rho_{2i}}-(h_{i}-\hat{h}_{i})\right).
% % 	\end{aligned}
% % \end{equation*}
...
% 	\begin{aligned}
% 		V & = V_2+ \sum_{i=1}^n \frac{1}{2\,\varrho_i}\,\tilde w_i^\mathsf{T}\tilde w_i+\sum_{i=1}^n \frac{1}{2}\,{\tilde{\alpha}}^2+\sum_{i=1}^n \left(\frac{1}{2\hbar_{1,i}}{\tilde d}^2_{1,i}+\frac{1}{2\hbar_{2,i}}{\tilde d}^2_{2,i}\right).
% 	\end{aligned}
% \end{equation}
...
% \begin{equation}\label{eq:46}
% 	\begin{aligned}
% 		\dot{V}  & =- \sum_{i=1}^{n}\varGamma_{1,i}z_{1,i}\Psi_{1,i} - \frac{1}{\omega_{1}}\sum_{i=1}^{n}\frac{k_{1,i} z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}- \frac{1}{\omega_{1}}\sum_{i=1}^{n} \frac{ z_{1,i}\hat d_{1,i}}{\rho_{1}^2 - z_{1,i}^2}\\
% 		& - \sum_{i=1}^{n}\varGamma_{2,i}z_{2,i}\Psi_{2,i}- \frac{1}{\omega_{2}}\sum_{i=1}^{n}\frac{ k_{2,i} z_{2,i}^2}{\rho_{2i}^2 - e_{2,i}^2} - \frac{1}{\omega_{2}}\sum_{i=1}^{n} \frac{ z_{2,i}\hat d_{2,i}}{\rho_{2}^2 - z_{2,i}^2}  \\
% 		& + \frac{\beta }{\omega_{2}}\sum_{i=1}^{n} \varGamma_{2,i} (h_{i}-\hat{h}_{i}) +\sum_{i=1}^n \frac{1}{\varrho_i}\,\tilde w_i^\mathsf{T}\,\dot{\tilde w}_i+\sum_{i=1}^{n}{{\tilde{\alpha}}}{}\,\dot {\tilde{\alpha}}+\sum_{i=1}^{n} \left(
% 			\frac{1}{\hbar_{1,i}}\,\tilde{d}_{1,i}\dot{\tilde{d}}_{1,i}
% 		  + \frac{1}{\hbar_{2,i}}\,\tilde{d}_{2,i}\dot{\tilde{d}}_{2,i}
...
% % \frac{1}{\hbar_{1,i}} \tilde{d}_{1,i} \dot{\tilde{d}}_{1,i} 
% % - \frac{1}{\omega_1} \frac{z_{1,i} \hat{d}_{1,i}}{\rho_{1}^2 - z_{1,i}^2}\\
% % &= \frac{1}{\hbar_{1,i}} \tilde{d}_{1,i}\dot{d}_{1,i} +\tilde{d}_{1,i} \operatorname{sgn}(s_{1,i})+\frac{\varpi_{1,i}}{\hbar_{1,i}} \tilde{d}_{1,i} {\hat d}_{1,i}- \frac{z_{1,i} \tilde{d}_{1,i}}{\omega_1(\rho_{1}^2 - z_{1,i}^2)}
% % + \frac{z_{1,i} \tilde{d}_{1,i}}{\omega_1(\rho_{1}^2 - z_{1,i}^2)}
% % - \frac{z_{1,i} d_{1,i}}{\omega_1(\rho_{1}^2 - z_{1,i}^2)}\\
% % % &\le \frac{1}{2\hbar_{1,i}} \tilde{d}_{1,i}^2 + \frac{ 1}{2\hbar_{1,i}} \dot{\bar{{d}}}_{1,i}^2+\frac{1}{2} \tilde{d}_{1,i}^2 + \frac{1}{2}-\frac{\varpi_{1,i}}{2 \hbar_{1,i}}{\tilde{d}_{1,i}}^2+\frac{\varpi_{1,i}}{2 \hbar_{1,i}}+\frac{\epsilon_1}{2 \omega_1} \frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}+\frac{1}{2\epsilon_1 \omega_1  (\rho_{1}^2 - z_{1,i}^2)} {\bar d}^2_{1,i}\\
% % &\le \left(\frac{1}{2\hbar_{1,i}}-\frac{\varpi_{1,i}}{2 \hbar_{1,i}}+ \frac{1}{2}\right)\tilde{d}_{1,i}^2+ \frac{ 1}{2\hbar_{1,i}} \dot{\bar{{d}}}_{1,i}^2+\frac{\varpi_{1,i}}{2 \hbar_{1,i}}{\bar d}^2_{1,i}+\frac{1}{2\epsilon_1 \omega_1  (\rho_{1}^2 - z_{1,i}^2)} {\bar d}^2_{1,i}+\frac{\epsilon_1}{2 \omega_1} \frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}+ \frac{1}{2}
% % \end{aligned}
% % \end{equation}
...
% % &=
% % \frac{1}{\hbar_{2,i}} \tilde{d}_{2,i} \dot{\tilde{d}}_{2,i} 
% % - \frac{1}{\omega_2} \frac{z_{2,i} \hat{d}_{2,i}}{\rho_{2}^2 - z_{2,i}^2}\\
% % &\le \left(\frac{1}{2\hbar_{2,i}}-\frac{\varpi_{2,i}}{2 \hbar_{2,i}}+ \frac{1}{2}\right)\tilde{d}_{2,i}^2+ \frac{ 1}{2\hbar_{2,i}} \dot{\bar{{d}}}_{2,i}^2+\frac{\varpi_{2,i}}{2 \hbar_{2,i}}{{d}^2_{2,i}}+\frac{1}{2 \epsilon_2\omega_2  (\rho_{2}^2 - z_{2,i}^2)} {d}_{2,i}^2+\frac{\epsilon_2}{2 \omega_2} \frac{z_{2,i}^2}{\rho_{2}^2 - z_{2,i}^2}+ \frac{1}{2}
% % \end{aligned}
% % \end{equation}
% Therefore,
% \begin{equation}\label{eq:51}
% 	\begin{aligned}
% 		\dot{V_d} & 
% 		= -\frac{1}{\omega_{1}}\sum_{i=1}^{n} \frac{ z_{1,i}\hat d_{1,i}}{\rho_{1}^2 - z_{1,i}^2} - \frac{1}{\omega_{2}}\sum_{i=1}^{n} \frac{ z_{2,i}\hat d_{2,i}}{\rho_{2}^2 - z_{2,i}^2}  +\sum_{i=1}^{n} \left(
% 			\frac{1}{\hbar_{1,i}}\,\tilde{d}_{1,i}\dot{\tilde{d}}_{1,i}
% 		  + \frac{1}{\hbar_{2,i}}\,\tilde{d}_{2,i}\dot{\tilde{d}}_{2,i}
% 	  \right)   \\   
% 	  &\le \left(\frac{1+\hbar_{1,i}-\varpi_{1,i}}{2\hbar_{1,i}}\right)\tilde{d}_{1,i}^2 +\left(\frac{1+\hbar_{2,i}-\varpi_{2,i}}{2\hbar_{2,i}}\right)\tilde{d}_{2,i}^2+\frac{\epsilon_1}{2 \omega_1} \frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}+\frac{\epsilon_2}{2 \omega_2} \frac{z_{2,i}^2}{\rho_{2}^2 - z_{2,i}^2}+H,
% 	\end{aligned}
% \end{equation}
% where 
% \[ H = \frac{1}{2\hbar_{1,i}} \dot{\bar{{d}}}_{1,i}^2+\frac{\varpi_{1,i}}{2 \hbar_{1,i}}{\bar d}^2_{1,i}+\frac{1}{2\epsilon_1 \omega_1  (\rho_{1}^2 - z_{1,i}^2)} {\bar d}^2_{1,i}+ \frac{ 1}{2\hbar_{2,i}} \dot{\bar{{d}}}_{2,i}^2+\frac{\varpi_{2,i}}{2 \hbar_{2,i}}{{d}^2_{2,i}}+\frac{1}{2 \epsilon_2\omega_2  (\rho_{2}^2 - z_{2,i}^2)} {d}_{2,i}^2+1\].


% According to \cref{lemma:4}, there is
% \begin{equation}\label{eq:52}
% 	\begin{aligned}
% 		\ln\frac{\rho_{j,i}^2}{\rho_{j,i}^2-z_{j,i}^2}\le \frac{z_{j,i}^2}{\rho_{j,i}^2-z_{j,i}^2}.
% 	\end{aligned}
% \end{equation}
...
% 	\begin{aligned}
% 		\dot{V} &\le  - \frac{1}{\omega_{1}}\sum_{i=1}^{n}\frac{k_{1,i} z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}- \frac{1}{\omega_{2}}\sum_{i=1}^{n}\frac{ k_{2,i} z_{2,i}^2}{\rho_{2i}^2 - z_{2,i}^2} - \frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \|\tilde w_i\|^2-\frac{\lambda}{2} \sum_{i=1}^{n} {\tilde{\alpha}}^{2}+\left(\frac{1+\hbar_{1,i}-\varpi_{1,i}}{2\hbar_{1,i}}\right)\tilde{d}_{1,i}^2 \\
% 		&+\left(\frac{1+\hbar_{2,i}-\varpi_{2,i}}{2\hbar_{2,i}}\right)\tilde{d}_{2,i}^2 +\frac{\epsilon_1}{2 \omega_1} \frac{z_{1,i}^2}{\rho_{1}^2 - z_{1,i}^2}+\frac{\epsilon_2}{2 \omega_2} \frac{z_{2,i}^2}{\rho_{2}^2 - z_{2,i}^2}
...
% 		 & \le\sum_{i=1}^{n} \left( - \frac{k_{1,i}}{\omega_{1}}+\frac{\epsilon_1}{2 \omega_1}\right)\ln\frac{\rho_{1}^2}{\rho_{1}^2-z_{1,i}^2}+\sum_{i=1}^{n} \left( - \frac{k_{2,i}}{\omega_{2}}+\frac{\epsilon_2}{2 \omega_2}\right)\ln\frac{\rho_{2}^2}{\rho_{2}^2-z_{2,i}^2}- \frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \|\tilde w_i\|^2-\frac{\lambda}{2} \sum_{i=1}^{n} {\tilde{\alpha}}^{2} \\
% 		 &+\left(\frac{1+\hbar_{1,i}-\varpi_{1,i}}{2\hbar_{1,i}}\right)\tilde{d}_{1,i}^2 +\left(\frac{1+\hbar_{2,i}-\varpi_{2,i}}{2\hbar_{2,i}}\right)\tilde{d}_{2,i}^2
% 		 +\frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \bar w_i^2 +\frac{1}{2\lambda} \sum_{i=1}^{n}R_{i}^2 + \frac{n}{4  \omega_{1} \iota_1 }+ \frac{n}{4  \omega_{2}\iota_2 }+H .  
% 	\end{aligned}
% \end{equation}
...
% From the above \cref{eq:53,eq:54,eq:55,eq:56}, as long as the controller parameters are chosen reasonably, $V$ is converge exponentially, which implies all signals $z_{1,i}, z_{2,i}, {\tilde{\alpha}}, \tilde w_i, \tilde{d}_{1,i}, \tilde{d}_{2,i}$ are bounded within the compact set $\mathcal{Q} $. Let
% \begin{equation}\label{eq:57}
% 	\mathcal{Q} =\left\{ (z_{1,i},z_{2,i},\tilde\alpha,\tilde w_i,\tilde d_{1,i},\tilde d_{2,i})\,:\;
...
% 	&-\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left[ \left(\frac{1}{2\,\varrho_i}\,\tilde w_i^2\right)^{\frac{\eta}{2}-1}+\left(\frac{1}{2}\,{\tilde{\alpha}}^2\right)^{\frac{\eta}{2}-1}+\left(\frac{1}{2\hbar_{1,i}}{\tilde d}^2_{1,i}\right)^{\frac{\eta}{2}-1}+\left(\frac{1}{2\hbar_{2,i}}{\tilde d}^2_{2,i}\right)^{\frac{\eta}{2}-1}\right] \\
% 		        & - n^{\frac\eta2}\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left[
% 		\left(\frac{1}{2\,\varrho_i}\,\tilde w_i^2\right)^{\frac{\eta}{2}+1}+\left(\frac{1}{2}\,{\tilde{\alpha}}^2\right)^{\frac{\eta}{2}+1}+\left(\frac{1}{2\hbar_{1,i}}{\tilde d}^2_{1,i}\right)^{\frac{\eta}{2}+1}+\left(\frac{1}{2\hbar_{2,i}}{\tilde d}^2_{2,i}\right)^{\frac{\eta}{2}+1} \right]   \\
% 		&        
...
% % +   \left( \frac{z_{2,i}^{2}}{2(\rho_{2i}^{2} - z_{2,i}^{2})} \right)^{1 + \frac{\eta}{2}}   + \left(\frac{1}{2\,\varrho_i}\,\tilde w_i^2\right)^{\frac{\eta}{2}+1}+\left(\frac{1}{2}\,{\tilde{\alpha}}^2\right)^{\frac{\eta}{2}+1}+\left(\frac{1}{2\,k_{I i}}\,\sigma_{i}^2\right)^{\frac{\eta}{2}+1} \right)\\
...
% 		+ \sum_{i=1}^{n}\frac{1}{2\,\varrho_i}\,\tilde w_i^2+\sum_{i=1}^{n}\frac{1}{2}\,{\tilde{\alpha}}^2+\frac{1}{2\hbar_{1,i}}{\tilde d}^2_{1,i}+\frac{1}{2\hbar_{2,i}}{\tilde d}^2_{2,i}\right)^{1-\frac{\eta}{2}} \\
