...
Let $T_p>0$ be the predefined convergence time and $0<p<1$. Define 
 ...
 $$
 b(t)=\left(\frac{\max\{t,t_\varepsilon\}}{T_p}\right)^p \in [b_{\min},1],
 $$
...
Q_{j,i}=\frac{ z_{j,i}^4}{(\rho_{j,i}^2-z_{j,i}^2)^2}>0,
 \rho_{j,i}^2 - z_{j,i}^2 \le \rho_{j,i}^2,
\Phi_{i}=\frac{z_{j,i}\left(\rho_{j,i}^{2}-z_{j,i}^{2}\right)}{\rho_{j,i}^{4}}
...
\mathcal{K}_{1,i}(z_{j,i},\rho_{j,i})
=\frac{2\pi}{\eta T_p}\left[(V_{j,i})^{-\eta/2}+n^{-\frac{\eta}{2} }(V_{j,i})^{\eta/2}\right]$.

则一步 Lyapunov 导数化简为

$$
\dot V_1
\le \sum_{i=1}^n \left( P_{1,i}z_{1,i} z_{2,i}-
2\mathcal{K}_{1,i} V_{1,i}-P_{1,i}z_{1,i}\tilde\alpha_i- k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2} 
\right).
$$
...
\begin{equation}
	\begin{aligned}
		...
		&
		 \le  \sum_{i=1}^n \left[P_{1,i}z_{1,i} z_{2,i}+ z_{2,i}\,\left(f_i(x)+\omega_i(x,t)+g_i(u^{\prime}_i-\Delta u_i)-\dot\alpha_i^{f}-\delta_i\ +\ \mu\,\zeta_i\ +\ g_i\,\Delta u_i \right)\right]
	\end{aligned}
\end{equation}
...
\begin{aligned}
	u^{\prime} =&C(q,\dot{q})x_2 + G(q)+M(q)\left[\dot\alpha^{f}+\delta -\mu\,\zeta-P_1 z_1-\mathcal{K}_{2}
-\hat \omega
-k_s\,\mathrm{sgn}(z_{2})-k_{2}z_2\right]
\end{aligned}
\end{equation}
where $k_2=\mathrm{diag}(k_{2,1},k_{2,i},\cdots,k_{2,n})$, $k_{2,i}>0$, $k_s>0$, $\mathcal{K}_{2}=\frac{\pi}{2\eta T_p}\left[ 2^{\eta/2} | z_2\vert ^{1-\eta}\mathrm{sgn}(z_2)+(\frac{n}{2})^{{\eta}/{2} }| z_2\vert ^{1+\eta}\mathrm{sgn}(z_2)\right]$.
