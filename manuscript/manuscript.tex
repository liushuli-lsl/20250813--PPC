...
% \begin{equation}
% 	h_i={\theta_{i}^*}^{T} { Q_{2,i}} (\chi)+\varepsilon_i, 
% 	\label{eq:4}
% \end{equation}
% where ${{\theta_{i}^*}}\in \mathbb{R}^N, \theta_{i}^*=[{{w}_{1,i}},{{w}_{2,i}},\cdots,{{w}_{i,N}}]^{T}$ is the idea weight vector, $Q_{2,i}(\chi)\!\in\mathbb{R}^N$ is the vector of basis functions, and $\varepsilon_i$ is the approximation error satisfying $|\varepsilon_i|\le \bar\varepsilon$ for all $\chi$ in a compact set $\Omega_\chi\subset\mathbb{R}^m$, with a known constant $\bar\varepsilon>0$. ${ Q_{2,i}}(\chi)=[{{ Q}_{1,i}( \chi )}, {{ Q_{2,i}}( \chi )}, \cdots,{{ Q_{i,N}}( \chi )}]^{T}$ is the activation function of the hidden layer neurons. The base function \( { Q}_{i,\ell }( \chi ) \) is
% \begin{equation}
...
% \begin{equation}
% 	\theta_{i}^* = \arg \min_{\theta_{i} \in \mathbb{R}^N} \left\{ \sup_{\chi \in \Omega_\chi} | f_i(\chi) - \theta_{i}^{\top}  Q_{2,i}(\chi) | \right\}.
% 	\label{eq:6}
% \end{equation}
...
\le \bar \theta_{i}\|\tilde \theta_{i}\| - \|\tilde \theta_{i}\|^2, \dot{\tilde \theta}_i=-\dot{\hat \theta}_i$, and combined with Young inequalities $\bar \theta_{i}\|\tilde \theta_{i}\| \le \frac12\|\tilde \theta_{i}\|^{2}
	...
	\begin{equation}\label{eq:47}
		\begin{aligned}
				&\sum_{i=1}^{n}
					\frac{1}{\varrho_i}\,\tilde \theta_{i}^{\top}\dot{\tilde \theta}_i  \le
...
在 BLF 约束管道内 $|z_{1,i}(t)|<\rho_{1,i}(t)$ 不变，故存在常数 $\delta\in(0,1)$ 使$\frac{|z_{1,i}(t)|}{\rho_{1,i}(t)}\le 1-\delta, \forall t \in (0,+\infty)$. 令 $c_\delta=\frac{1}{(2\delta-\delta^2)^2}>0$, 可得到

% 令 $c_\delta=\frac{1}{(2\delta-\delta^2)^2}>0$。则对任意 $\varepsilon_i>0$ 有
...
% +\frac{c_\delta^{,2}}{2\varepsilon_i},\tilde\alpha_i^2 .
% \end{aligned}
% \end{equation}

 
$$
|P_{1,i}z_{1,i}| \le c_\delta |z_{1,i}|\le c_\delta \sqrt{2V_{1,i}},
$$
$$
|P_{1,i}z_{1,i}\tilde\alpha_i|
\le c_\delta\sqrt{2V_{1,i}}\,|\tilde\alpha_i|
\le \varepsilon_i V_{1,i}+\frac{c_\delta^{\,2}}{2\varepsilon_i}\tilde\alpha_i^2,
$$


由 $\lambda\dot\alpha^f=-(\alpha^f-\alpha)$ 得 $\dot{\tilde\alpha}=\dot\alpha-\tfrac{1}{\lambda}\tilde\alpha$，从而

$$
\tilde\alpha^\top\dot{\tilde\alpha}
\le -\frac{1}{2\lambda}\|\tilde\alpha\|^2+\lambda\,\Xi_\alpha(t),
$$

% From \cref{eq:22,eq:23}, the time derivative of the filtering error $\dot{\tilde{\alpha}}$ is given as
% \begin{equation}\label{eq:48}
% 	\tilde\alpha^\top \dot{\tilde \alpha}= {\tilde{\alpha}}\dot{\alpha}-\frac{1}{\lambda}{\tilde{\alpha}}^2,
% \end{equation}
...
% 	\begin{aligned}
% 		{\tilde{\alpha}}\dot{\tilde{\alpha}}\leq -\frac{\lambda}{2}  {\tilde{\alpha}}^{2} + \frac{1}{2\lambda} R_{i}^2. 
% 	\end{aligned}
% \end{equation}

其中 $\Xi_\alpha(t)$ 由有界信号构成。将其与 \eqref{eq:3} 相加，并取 $\varepsilon_i=k_{1,i}$ 可得
...
+\tilde\alpha^\top\dot{\tilde\alpha}\\
& \le
\frac{\varepsilon_i}{2} V_{1,i}+\frac{c_\delta^{\,2}}{2\varepsilon_i}\tilde\alpha_i^2-k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}-\frac{1}{2\lambda}\|\tilde\alpha\|^2+\lambda\,\Xi_\alpha(t)\\
&\le
(\frac{\varepsilon_i}{2}-2k_{1,i})V_{1,i}
-\Big(\frac{1}{2\lambda}-\frac{c_\delta^{\,2}}{2k_{1,i}}\Big)\tilde\alpha_i^2
+\lambda\,\Xi_\alpha(t)\\
&\le
-\tfrac{3}{2}k_{1,i}V_{1,i}
-\Big(\frac{1}{2\lambda}-\frac{c_\delta^{\,2}}{2k_{1,i}}\Big)\tilde\alpha_i^2
+\lambda\,\Xi_\alpha(t).
\end{aligned}
\end{equation}
...
若进一步选取
$
\boxed{\ \lambda\ \le\ \frac{k_{1,i}}{c_\delta^{\,2}}\ ,\ }
$
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		...
		\le&\sum_{i=1}^n \left( - k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}-\Psi(V_{1,i})\right)
		+\sum_{i=1}^n \left( - k_{2,i}\frac{\rho_{2,i}^2 z_{2,i}^2}{\rho_{2,i}^2-z_{2,i}^2}-\Psi(V_{2,i})  \right)\\
		&+\frac{\varepsilon_i}{2} V_{1,i}+\frac{c_\delta^{\,2}}{2\varepsilon_i}\tilde\alpha_i^2-\frac{1}{2\lambda}\|\tilde\alpha\|^2+\lambda\,\Xi_\alpha(t)-\Big(\big(1-\tfrac{\varepsilon}{2}\big)\lambda_{o,\min}-\tfrac{1}{4k_s}\Big)\|\tilde\omega\|^2
+\frac{1}{2\varepsilon}\big\|\Lambda_o^{-1/2}\Delta_\omega\big\|^2- \frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \|\tilde \theta_{i}\|^2+\frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \bar \theta_{i}^2\\
\le&\sum_{i=1}^n \left( - k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2}-\Psi(V_{1,i})\right)
		+\sum_{i=1}^n \left( - k_{2,i}\frac{\rho_{2,i}^2 z_{2,i}^2}{\rho_{2,i}^2-z_{2,i}^2}-\Psi(V_{2,i})  \right)\\
		&+\frac{\varepsilon_i}{2} V_{1,i}+\frac{c_\delta^{\,2}}{2\varepsilon_i}\tilde\alpha_i^2-\frac{1}{2\lambda}\|\tilde\alpha\|^2+\lambda\,\Xi_\alpha(t)-\Big(\big(1-\tfrac{\varepsilon}{2}\big)\lambda_{o,\min}-\tfrac{1}{4k_s}\Big)\|\tilde\omega\|^2
+\frac{1}{2\varepsilon}\big\|\Lambda_o^{-1/2}\Delta_\omega\big\|^2- \frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \|\tilde \theta_{i}\|^2+\frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \bar \theta_{i}^2
...
\begin{equation}\label{eq:45}
	\begin{aligned}
		\dot{V} 
\le&-(2k_{1,i}-\frac{\varepsilon_i}{2})V_{1,i}- 2k_{2,i}V_{2,i}-\Big(\frac{1}{2\lambda}-\frac{c_\delta^{\,2}}{2k_{1,i}}\Big)\tilde\alpha_i^2 -\Big(\big(1-\tfrac{\varepsilon}{2}\big)\lambda_{o,\min}-\tfrac{1}{4k_s}\Big)\|\tilde\omega\|^2\\
&- \frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \|\tilde \theta_{i}\|^2+\frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \bar \theta_{i}^2
+\frac{1}{2\varepsilon}\big\|\Lambda_o^{-1/2}\Delta_\omega\big\|^2
+\lambda\,\Xi_\alpha(t)
	\end{aligned}
...
\begin{aligned}
r = \min \left\{
	\min_{i}\left(2k_{1,i}-\frac{\varepsilon_i}{2}\right), 2k_{2,i}, \min_{i}\left(\frac{1}{2\lambda}-\frac{c_\delta^{\,2}}{2k_{1,i}}\right),\min_{i}\left(\big(1-\tfrac{\varepsilon}{2}\big)\lambda_{o,\min}-\tfrac{1}{4k_s}\right),
      \min_{i}(\kappa_i)
\right\}>0,
\end{aligned}
\end{equation}
\begin{equation}\label{eq:56}
	\sigma =\frac{\kappa_i}{2\varrho_i}\sum_{i=1}^n \bar \theta_{i}^2
+\frac{1}{2\varepsilon}\big\|\Lambda_o^{-1/2}\Delta_\omega\big\|^2
+\lambda\,\Xi_\alpha(t).
\end{equation} 
...
%     &\ \min_{i=1...n}\left(\lambda - \frac{r}{\rho_{1,i}^2-e_{1,i}^2}\right),\
%       \min_{i=1...n}\left(\frac{k_{I i}}{\rho_{1,i}^2 - e_{1,i}^2} - \frac{1}{k_{I i}}\right) 
% \Bigg\},
% \end{aligned}
% \end{equation}
...
From the above \cref{eq:53,eq:54,eq:55,eq:56}, as long as the controller parameters are chosen reasonably, $V$ is converge exponentially, which implies all signals $z_{1,i}, z_{2,i}, {\tilde{\alpha}}, \tilde \theta_{i}, \tilde{\omega}_{i}$ are bounded within the compact set $\mathcal{Q} $. Let
\begin{equation}\label{eq:57}
	\mathcal{Q} =\left\{ (z_{1,i}, z_{2,i}, {\tilde{\alpha}}, \tilde \theta_{i}, \tilde{\omega}_{i}\,:
	\|(z_1,z_2,\tilde\alpha, \tilde \theta, \tilde{\omega})\|\le \sqrt{\phi}\right\}.
\end{equation}
...
% 	\begin{aligned}
% 		\dot{V} 
% &\le-\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left( \left( \frac{z_{1,i}^{2}}{2(\rho_{1,i}^{2} - z_{1,i}^{2})} \right)^{1 - \frac{\eta}{2}} 
...
% &- n^{\frac\eta2}\frac{\pi}{\eta T_{p}}\sum_{i=1}^{n}\left(
%   \left( \frac{z_{1,i}^{2}}{2(\rho_{1,i}^{2} - z_{1,i}^{2})} \right)^{1 + \frac{\eta}{2}} 
