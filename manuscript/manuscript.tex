...
\delta\ =\ -\frac{\pi}{2\gamma T_\zeta}\left(|\zeta|^{1- \gamma}\mathrm{sgn}(\zeta)+|\zeta|^{1+\gamma}\mathrm{sgn}(\zeta)\right),
	\end{cases}
\end{equation}
其中 $0<\gamma<1,\ \mu_2>0$, $\mathrm{sgn}(\zeta)^{1\pm \gamma}=|\zeta|^{1\pm \gamma}\mathrm{sgn}(\zeta)$, 并取初值 $\zeta_{i}(0)=0$。
...
$$
\Phi_{1,i}=\frac{z_{j,i}\left(\rho_{j,i}^{2}-z_{j,i}^{2}\right)}{\rho_{j,i}^{2}}=z_{j,i}-\frac{z_{j,i}^{3}}{\rho_{j,i}^{2}},
$$
把它代回，可设计虚拟控制律为：
\begin{equation}\label{eq:25}
...
\mathcal{K}_{1,i}(z_{j,i},\rho_{j,i})
=\frac{2\pi}{\eta T_p}\Big((V_{j,i})^{-\eta/2}+n^{-\frac{\eta}{2} }(V_{j,i})^{\eta/2}\Big)$.
...
2\mathcal{K}V_{1,i}-P_{1,i}z_{1,i}\tilde\alpha_i- k_{1,i}\frac{\rho_{1,i}^2 z_{1,i}^2}{\rho_{1,i}^2-z_{1,i}^2} 
\Big].
$$
...
\begin{equation}
	\begin{aligned}
		...
		&
		 \le  \sum_{i=1}^n \left[P_{1,i}z_{1,i} z_{2,i}+ z_{2,i}\,\left(f_i(x)+\omega_i(x,t)+g_i(u^{\prime}_i-\Delta u_i)-\dot\alpha_i^{f}-\delta_i\ +\ \mu_2\,\zeta_i\ +\ g_i\,\Delta u_i \right)\right]
	\end{aligned}
\end{equation}
...
\begin{aligned}
	u^{\prime} =&C(q,\dot{q})x_2 + G(q)+M(q)\left[\dot\alpha^{f}+\delta -\mu_2\,\zeta-P_1 z_1-\mathcal{K}_{2}
-\hat \omega
-k_s\,\mathrm{sgn}(z_{2})-k_{2}z_2\right]
\end{aligned}
\end{equation}
where $k_2=\mathrm{diag}\{k_{2,1},k_{2,i},\cdots,k_{2,n}\}>0$, $k_{2,i}>0$, $k_s>0$, $\mathcal{K}_{2}=\frac{\pi}{2\eta T_p}\left[ 2^{\eta/2} | z_2\vert ^{1-\eta}\mathrm{sgn}(z_2)+(\frac{n}{2})^{{\eta}/{2} }| z_2\vert ^{1+\eta}\mathrm{sgn}(z_2)\right]$.
...
\begin{equation}
	\begin{aligned}
		...
		&
		 = \sum_{i=1}^n \left[\,P_{2,i}z_{2,i}\,\left(f_i(x)+\omega_i(x,t)+g_i(u^{\prime}_i-\Delta u_i)-\dot\alpha_i^{f}-\delta_i\ +\ \mu_2\,\zeta_i\ +\ g_i\,\Delta u_i-\tfrac{z_{2}^{3}}{\rho_{2,i}^{3}}\dot\rho_{2,i} \right)\right]
	\end{aligned}
\end{equation}
...
\Big]+\sum_{i=1}^n \left( - k_{2,i} z_{2,i}^2-z_{2,i}\mathcal{K}_{2,i}-z_{2,i}\tilde{\omega}_i  \right) -k_s\sum_{i=1}^n z_{2,i}\mathrm{sgn}(z_2)
\end{aligned}
\end{equation}
